...
void CDibBar::Invalidate() {
  RECT rect = {static_cast<long>(0), static_cast<long>(0),
               static_cast<long>(m_dwWidth), static_cast<long>(m_dwHeight)};
...
void CDibBar::__DivideTextureSize(DWORD dwSize, DWORD dwMax, DWORD *pdwxStep,
                                  DWORD *pdwxCount, DWORD *pdwxRest) {
  if (dwSize < dwMax) {
    *pdwxStep = dwMax;
    *pdwxCount = 0;
...
CBlockTexture *CDibBar::__BuildTextureBlock(DWORD dwxPos, DWORD dwyPos,
                                            DWORD dwImageWidth,
                                            DWORD dwImageHeight,
...
void CDibBar::__BuildTextureBlockList(DWORD dwWidth, DWORD dwHeight,
                                      DWORD dwMax) {
  ...
  for (DWORD y = 0; y < dwyCount; ++y) {
    for (DWORD x = 0; x < dwxCount; ++x) {
      CBlockTexture *pTexture = __BuildTextureBlock(
          x * dwxStep, y * dwyStep, dwxStep, dwyStep, dwMax, dwMax);
      if (pTexture)
        m_kVec_pkBlockTexture.push_back(pTexture);
    }

    CBlockTexture *pTexture = __BuildTextureBlock(
        dwxCount * dwxStep, y * dwyStep, dwxRest, dwyStep, dwxTexRest, dwMax);
  ...
  for (DWORD x = 0; x < dwxCount; ++x) {
    CBlockTexture *pTexture = __BuildTextureBlock(
        x * dwxStep, dwyCount * dwyStep, dwxStep, dwyRest, dwMax, dwyTexRest);
    if (pTexture)
      m_kVec_pkBlockTexture.push_back(pTexture);
  }

  if (dwxRest > 0 && dwyRest > 0) {
    CBlockTexture *pTexture =
        __BuildTextureBlock(dwxCount * dwxStep, dwyCount * dwyStep, dwxRest,
                            dwyRest, dwxTexRest, dwyTexRest);
    if (pTexture)
      m_kVec_pkBlockTexture.push_back(pTexture);
  }
}
