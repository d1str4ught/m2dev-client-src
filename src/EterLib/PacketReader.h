...
// PacketReader: Safe, bounds-checked packet deserialization from a read-only
// buffer. Used to parse incoming packets after the dispatch loop validates the
// header and length.
//
// Usage:
...
public:
  PacketReader(const void *data, size_t size)
      : m_buf(static_cast<const uint8_t *>(data)), m_size(size), m_pos(0) {
  ...
  template <typename T> bool ReadStruct(T &out) {
