...
int CGraphicTextInstance::Hyperlink_GetText(char *buf, int len) {
  ...
  int written = WideCharToMultiByte(
      CP_UTF8, WC_ERR_INVALID_CHARS, gs_hyperlinkText.c_str(),
      (int)gs_hyperlinkText.length(), buf, len, nullptr, nullptr);

  return (written > 0) ? written : 0;
}

int CGraphicTextInstance::__DrawCharacter(CGraphicFontTexture *pFontTexture,
                                          wchar_t text, DWORD dwColor,
                                          wchar_t prevChar) {
  CGraphicFontTexture::TCharacterInfomation *pInsCharInfo =
      pFontTexture->GetCharacterInfomation(text);
...
void CGraphicTextInstance::__GetTextPos(DWORD index, float *x, float *y) {
  ...
  for (DWORD i = 0; i < index; ++i) {
    ...
    fFontMaxHeight =
        std::max(float(m_pCharInfoVector[i]->height), fFontMaxHeight);
  }
...
void CGraphicTextInstance::Update() {
  ...
  if (!m_roText.IsNull() && !m_roText->IsEmpty()) {
    ...
    if (pFontTexture) {
      CGraphicFontTexture::TCharacterInfomation *pSpaceInfo =
          pFontTexture->GetCharacterInfomation(L' ');
  ...
  auto ResetState = [&, spaceHeight]() {
    ...
    m_textWidth = 0;
    m_textHeight =
        spaceHeight; // Use space height instead of 0 for cursor rendering
  ...
  int wTextLen =
      MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8, utf8Len,
                          wTextBuf.data(), (int)wTextBuf.size());

  // If strict UTF-8 conversion fails, try lenient mode (replaces invalid
  // sequences)
  if (wTextLen <= 0) {
    // Try lenient conversion (no MB_ERR_INVALID_CHARS flag)
    wTextLen = MultiByteToWideChar(CP_UTF8, 0, utf8, utf8Len, wTextBuf.data(),
                                   (int)wTextBuf.size());
  ...
  // These functions handle Arabic shaping, BiDi reordering, and chat formatting
  // properly
  ...
  if (m_isChatMessage && !m_chatName.empty() && !m_chatMessage.empty()) {
    ...
    if (!msgHasTags) {
      ...
      std::vector<wchar_t> visual =
          BuildVisualChatMessage(wName.data(), (int)wName.size(), wMsg.data(),
                                 (int)wMsg.size(), m_computedRTL);
      ...
      if (wTextLen <= 0) {
        wTextLen = MultiByteToWideChar(CP_UTF8, 0, utf8, utf8Len,
                                       wTextBuf.data(), (int)wTextBuf.size());
        if (wTextLen <= 0) {
          ResetState();
          return;
  ...
  const bool hasTags = (std::find(wTextBuf.begin(), wTextBuf.begin() + wTextLen,
                                  L'|') != (wTextBuf.begin() + wTextLen));
  bool hasRTL = false;
  for (int i = 0; i < wTextLen; ++i) {
  ...
  // Tag-aware BiDi rendering: Parse tags, apply BiDi per segment, track
  // colors/hyperlinks OPTIMIZED: Use helper lambda to eliminate code
  // duplication (was repeated 5+ times)
  if (hasRTL || hasTags) {
    DWORD currentColor = dwColor;
    int hyperlinkStep =
        0; // 0=normal, 1=collecting metadata, 2=visible hyperlink
    ...
    // In chat RTL, force RTL base direction so prefixes like "[hyperlink]"
    // don't flip the paragraph to LTR.
    const bool forceRTLForBidi = (m_isChatMessage && m_computedRTL);

    // OPTIMIZED: Single helper function for flushing segments (eliminates 5x
    // code duplication)
    auto FlushSegment = [&](DWORD segColor) -> int {
      ...
      std::vector<wchar_t> visual = BuildVisualBidiText_Tagless(
          s_currentSegment.data(), (int)s_currentSegment.size(),
          forceRTLForBidi);

      wchar_t prevCh =
          m_pCharInfoVector.empty() ? 0 : 0; // no prev across segments
    ...
    // Prepend glyphs to the already-built draw list (used to place hyperlink
    // before message in RTL chat).
    auto PrependGlyphs = [&](CGraphicFontTexture *pFontTexture,
                             const std::vector<wchar_t> &chars, DWORD color,
                             int &outWidth) {
      outWidth = 0;

      static std::vector<CGraphicFontTexture::TCharacterInfomation *>
          s_newCharInfos;
      ...
      m_pCharInfoVector.insert(m_pCharInfoVector.begin(),
                               s_newCharInfos.begin(), s_newCharInfos.end());
      m_dwColorInfoVector.insert(m_dwColorInfoVector.begin(),
                                 s_newColors.begin(), s_newColors.end());
      m_kernVector.insert(m_kernVector.begin(), s_newKerns.begin(),
                          s_newKerns.end());
    ...
    for (int i = 0; i < wTextLen;) {
      ...
      if (tagType == TEXT_TAG_COLOR) {
        ...
        if (hyperlinkStep == 1) {
          ...
          currentHyperlink.sx =
              currentHyperlink.ex; // Start hyperlink at current cursor position
        } else if (hyperlinkStep == 2) {
          // End of visible section - render hyperlink text with proper Arabic
          // handling In RTL chat: we want the hyperlink chunk to appear BEFORE
          // the message, even if logically appended.
          if (!s_currentSegment.empty()) {
            ...
            if (openBracket >= 0 && closeBracket > openBracket) {
              ...
              s_content.assign(s_currentSegment.begin() + openBracket + 1,
              ...
              s_visibleToRender.insert(s_visibleToRender.end(), visual.begin(),
                                       visual.end());
            ...
            // Ensure a space AFTER the hyperlink chunk (so it becomes
            // "[hyperlink] اختبار...")
            s_visibleToRender.push_back(L' ');

            // Key behavior:
            // In RTL chat, place hyperlink BEFORE the message by prepending
            // glyphs.
            if (m_isChatMessage && m_computedRTL) {
              int addedWidth = 0;
              PrependGlyphs(pFontTexture, s_visibleToRender, currentColor,
                            addedWidth);
              ...
              for (size_t j = 0; j < s_visibleToRender.size(); ++j) {
                int w = __DrawCharacter(pFontTexture, s_visibleToRender[j],
                                        currentColor, prevCh);
      ...
      } else // TEXT_TAG_PLAIN or TEXT_TAG_TAG
      {
        if (hyperlinkStep == 1) {
          ...
          // Will be BiDi-processed for normal text, or rendered directly for
          // hyperlinks
...
void CGraphicTextInstance::Render(RECT *pClipRect) {
  ...
  // Use the computed direction for this text instance, not the global UI
  // direction
  if (m_computedRTL) {
    switch (m_hAlign) {
  ...
  static std::unordered_map<LPDIRECT3DTEXTURE9, std::vector<SVertex>>
      s_outlineBatches;
  static std::unordered_map<LPDIRECT3DTEXTURE9, std::vector<SVertex>>
      s_mainBatches;
  ...
  // LCD subpixel rendering: mask alpha writes to prevent corruption during
  // two-pass blending
...

  {
    ...
    if (m_isOutline) {
      ...
      for (i = m_pCharInfoVector.begin(); i != m_pCharInfoVector.end();
           ++i, ++charIdx) {
        pCurCharInfo = *i;

        float fKern =
            (charIdx < (int)m_kernVector.size()) ? m_kernVector[charIdx] : 0.0f;
        fCurX += fKern;
        ...
        std::vector<SVertex> &vtxBatch =
            s_outlineBatches[pFontTexture->GetD3DTexture()];
        ...
        akVertex[3].color = akVertex[2].color = akVertex[1].color =
            akVertex[0].color = m_dwOutLineColor;
        ...
        vtxBatch.push_back(akVertex[0]);
        vtxBatch.push_back(akVertex[1]);
        vtxBatch.push_back(akVertex[2]);
        vtxBatch.push_back(akVertex[2]);
        vtxBatch.push_back(akVertex[1]);
        vtxBatch.push_back(akVertex[3]);
    ...
    for (int i = 0; i < (int)m_pCharInfoVector.size(); ++i) {
      ...
      pFontTexture->SelectTexture(pCurCharInfo->index);
      std::vector<SVertex> &vtxBatch =
          s_mainBatches[pFontTexture->GetD3DTexture()];
      ...
      akVertex[0].color = akVertex[1].color = akVertex[2].color =
          akVertex[3].color = m_dwColorInfoVector[i];
  ...
  auto DrawBatchLCD = [](const std::unordered_map<
                             LPDIRECT3DTEXTURE9, std::vector<SVertex>> &batches,
                         bool skipPass2) {
    for (const auto &[pTexture, vtxBatch] : batches) {
      ...
      STATEMANAGER.SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);
      STATEMANAGER.DrawPrimitiveUP(D3DPT_TRIANGLELIST, vtxBatch.size() / 3,
                                   vtxBatch.data(), sizeof(SVertex));
  ...
  // Draw outline batches first (skip Pass 2 for black outlines — MODULATE with
  // black = 0)
  ...
  if (m_isCursor) {
    ...
    if (ulbegin < ulend) {
      ...
      STATEMANAGER.DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, 4, 2,
                                          c_FillRectIndices, D3DFMT_INDEX16,
  ...
  if (m_hyperlinkVector.size() != 0) {
    // FOR_ARABIC_ALIGN: RTL text is drawn with offset (m_v3Position.x -
    // m_textWidth) Use the computed direction for this text instance, not the
    // global UI direction
    int textOffsetX = m_computedRTL ? (int)(m_v3Position.x - m_textWidth)
                                    : (int)m_v3Position.x;
...
void CGraphicTextInstance::DestroySystem() { ms_kPool.Destroy(); }

CGraphicTextInstance *CGraphicTextInstance::New() { return ms_kPool.Alloc(); }
...
void CGraphicTextInstance::ShowCursor() { m_isCursor = true; }

void CGraphicTextInstance::HideCursor() { m_isCursor = false; }

void CGraphicTextInstance::ShowOutLine() { m_isOutline = true; }

void CGraphicTextInstance::HideOutLine() { m_isOutline = false; }
...
void CGraphicTextInstance::SetSecret(bool Value) { m_isSecret = Value; }

void CGraphicTextInstance::SetOutline(bool Value) { m_isOutline = Value; }
...
void CGraphicTextInstance::SetMultiLine(bool Value) { m_isMultiLine = Value; }

void CGraphicTextInstance::SetHorizonalAlign(int hAlign) { m_hAlign = hAlign; }

void CGraphicTextInstance::SetVerticalAlign(int vAlign) { m_vAlign = vAlign; }

void CGraphicTextInstance::SetMax(int iMax) { m_iMax = iMax; }
...
void CGraphicTextInstance::SetChatValue(const char *c_szName,
                                        const char *c_szMessage) {
  ...
  // Use BuildVisualChatMessage in Update() instead of
  // BuildVisualBidiText_Tagless
...
WORD CGraphicTextInstance::GetTextLineCount() {
  ...
  float fx = 0.0f;
  WORD wLineCount = 1;
  for (itor = m_pCharInfoVector.begin(); itor != m_pCharInfoVector.end();
       ++itor) {
    pCurCharInfo = *itor;
...
int CGraphicTextInstance::PixelPositionToCharacterPosition(int iPixelPosition) {
  ...
  for (int i = 0; i < (int)m_pCharInfoVector.size(); ++i) {
    ...
    // Use advance instead of width (width is not reliable for cursor
    // hit-testing)
  ...
  if (!m_visualToLogicalPos.empty() && visualPos >= 0 &&
      visualPos < (int)m_visualToLogicalPos.size())
    return m_visualToLogicalPos[visualPos];

  return visualPos;
}

int CGraphicTextInstance::GetHorizontalAlign() { return m_hAlign; }
...
void CGraphicTextInstance::__Initialize() {
  ...
  m_isUpdate = false;
  // Use Auto direction for input fields - they should auto-detect based on
  // content Only chat messages should be explicitly set to RTL
...
CGraphicTextInstance::CGraphicTextInstance() { __Initialize(); }

CGraphicTextInstance::~CGraphicTextInstance() { Destroy(); }
