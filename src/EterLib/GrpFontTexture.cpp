...
CGraphicFontTexture::CGraphicFontTexture() { Initialize(); }

CGraphicFontTexture::~CGraphicFontTexture() { Destroy(); }
...
bool CGraphicFontTexture::IsEmpty() const { return m_ftFace == nullptr; }
...
bool CGraphicFontTexture::CreateDeviceObjects() {
  ...
  // We keep m_charInfoMap keys but clear the entries so glyphs get
  // re-rasterized.
...
bool CGraphicFontTexture::Create(const char *c_szFontName, int fontSize,
                                 bool bItalic) {
  Destroy();
  ...
  if (!m_ftFace) {
    TraceError("CGraphicFontTexture::Create - Failed to create face for '%s'",
               c_szFontName ? c_szFontName : "(null)");
    return false;
  }
...
bool CGraphicFontTexture::UpdateTexture() {
  ...
  for (int y = 0; y < m_atlasHeight;
       ++y, pdwDst += pitch, pdwSrc += m_atlasWidth) {
    memcpy(pdwDst, pdwSrc, m_atlasWidth * sizeof(DWORD));
  }

  pFontTexture->Unlock();

  // DX11: Copy atlas buffer directly to DX11 texture (bypass DX9)
  {static int logCount=0; if(logCount<10){DWORD nonZero=0;for(int i=0;i<m_atlasWidth*m_atlasHeight;i++){if(m_pAtlasBuffer[i]){nonZero++;break;}}FILE*f=fopen("dx11_font.log","a");if(f){fprintf(f,"UpdateTex: dirty=%d dx11=%p srv=%p nonZero=%d\n",1,pFontTexture->GetDX11Texture(),pFontTexture->GetDX11SRV(),nonZero);fflush(f);fclose(f);}logCount++;}}
  ID3D11Texture2D *pDX11Tex = (ID3D11Texture2D *)pFontTexture->GetDX11Texture();
  if (pDX11Tex && ms_pD3D11Context) {
    D3D11_MAPPED_SUBRESOURCE mapped;
    if (SUCCEEDED(ms_pD3D11Context->Map(pDX11Tex, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) {
      const BYTE *pSrc = (const BYTE *)m_pAtlasBuffer;
      BYTE *pDst = (BYTE *)mapped.pData;
      UINT rowBytes = m_atlasWidth * sizeof(DWORD);
      for (int y = 0; y < m_atlasHeight; ++y) {
        memcpy(pDst + y * mapped.RowPitch, pSrc + y * rowBytes, rowBytes);
      }
      ms_pD3D11Context->Unmap(pDX11Tex, 0);
        {static int logCount2=0; if(logCount2<3){FILE*f=fopen("dx11_font.log","a");if(f){DWORD firstPx=m_pAtlasBuffer[0];DWORD midPx=m_pAtlasBuffer[m_atlasWidth*m_atlasHeight/2];fprintf(f,"  Map OK rowPitch=%u firstPx=0x%08X midPx=0x%08X\n",(unsigned)mapped.RowPitch,firstPx,midPx);fflush(f);fclose(f);}logCount2++;}}
    }
  }
  return true;
}
...
float CGraphicFontTexture::GetKerning(wchar_t prev, wchar_t cur) {
  ...
  FT_Vector delta;
  if (FT_Get_Kerning(m_ftFace, prevIndex, curIndex, FT_KERNING_DEFAULT,
                     &delta) != 0)
    return 0.0f;
...
CGraphicFontTexture::GetCharacterInfomation(wchar_t keyValue) {
...
CGraphicFontTexture::TCharacterInfomation *
CGraphicFontTexture::UpdateCharacterInfomation(TCharacterKey keyValue) {
  if (!m_ftFace)
    return NULL;
  ...
  int glyphBitmapWidth =
      bitmap.width / 3; // LCD bitmap is 3x wider (R,G,B per pixel)
  ...
  // Copy LCD subpixel FreeType bitmap into atlas buffer (R,G,B per-channel
  // coverage)
...
void CGraphicFontTexture::SelectTexture(DWORD dwTexture) {
  ...
  // DX11: Also update the SRV so DX11 rendering uses the font texture
  m_pDX11SRV = m_pFontTextureVector[dwTexture]->GetDX11SRV();
}
