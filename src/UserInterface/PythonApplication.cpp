...
CPythonApplication::CPythonApplication()
    : m_bCursorVisible(TRUE), m_bLiarCursorOn(false),
      m_iCursorMode(CURSOR_MODE_HARDWARE), m_isWindowed(false),
      m_isFrameSkipDisable(false), m_poMouseHandler(NULL), m_dwUpdateFPS(0),
      m_dwRenderFPS(0), m_fAveRenderTime(0.0f), m_dwFaceCount(0),
      m_fGlobalTime(0.0f), m_fGlobalElapsedTime(0.0f), m_dwLButtonDownTime(0),
      m_dwLastIdleTime(0), m_IsMovingMainWindow(false) {
#ifndef _DEBUG
...
void CPythonApplication::Exit() { PostQuitMessage(0); }
...
void CPythonApplication::UpdateGame() {
  ...
  m_GameEventManager.SetCenterPosition(kPPosMainActor.x, kPPosMainActor.y,
                                       kPPosMainActor.z);
  ...
  //        ÀÌ ºÎºÐ ¶§¹®¿¡ ¸ÞÀÎ ÄÉ¸¯ÅÍÀÇ Sound°¡ ÀÌÀü À§Ä¡¿¡¼­ ÇÃ·¹ÀÌ µÇ´Â
  //        Çö»óÀÌ ÀÖ¾úÀ½.
  m_pyPlayer.NEW_GetMainActorPosition(&kPPosMainActor);
...
bool CPythonApplication::Process() {
  ...
  //	//±×¸®°í m_dwCurUpdateTime´Â deltaÀÎµ¥ delta¶û absolute timeÀÌ¶û
  // ºñ±³ÇÏ¸é ¾îÂ¼ÀÚ´Â°Ü?
  //	//if (dt >= 500 || m_dwCurUpdateTime > s_uiNextFrameTime)

  //	//±âÁ¸ÄÚµå´ë·Î ÇÏ¸é 0.5ÃÊ ÀÌÇÏ Â÷ÀÌ³­ »óÅÂ·Î update°¡ Áö¼ÓµÇ¸é °è¼Ó
  // rendering frame skip¹ß»ý 	if (dt >= 500 || m_dwCurUpdateTime >
  // s_uiNextFrameTime)
  //	{
  ...
  if (!s_bFrameSkip) {
    ...
    //		CGrannyMaterial::TranslateSpecularMatrix(fabs(sin(pos)*0.005),
    // fabs(cos(pos)*0.005), 0.0f); 		pos+=0.01f;

    CGrannyMaterial::TranslateSpecularMatrix(g_specularSpd, g_specularSpd,
                                             0.0f);
    ...
    if (canRender) [[likely]] {
      ...
      if (m_pyGraphic.Begin()) [[likely]] {
        ...
        if (dwRenderEndTime - s_dwRenderCheckTime > 1000) [[unlikely]] {
          m_fAveRenderTime =
              float(double(s_dwRenderRangeTime) / double(s_dwRenderRangeFrame));
        ...
        if (dwCurFaceCount > 5000) {
          ...
          if (-1 == m_iForceSightRange) {
            ...
            float fRatio = 0.3f;
            s_fAveRenderTime =
                (s_fAveRenderTime * (100.0f - fRatio) +
                 std::max(16.0f, (float)m_dwCurRenderTime) * fRatio) /
                100.0f;
            ...
            float fDistance = std::max(
                (float)(fNear + (fFar - fNear) * (dbAvePow) / dbMaxPow), fNear);
...
bool CPythonApplication::CreateDevice(int width, int height, int Windowed,
                                      int bit, int frequency) {
  int iRet;

  m_grpDevice.InitBackBufferCount(2);
  iRet = m_grpDevice.Create(GetWindowHandle(), width, height,
                            Windowed ? true : false, bit, frequency);
...
bool LoadLocaleData(const char *localePath) {
  ...
  snprintf(szItemList, sizeof(szItemList), "%s/item_list.txt",
           GetLocalePathCommon());
  ...
  snprintf(szSkillDescFileName, sizeof(szSkillDescFileName), "%s/SkillDesc.txt",
           localePath);
  snprintf(szSkillTableFileName, sizeof(szSkillTableFileName),
           "%s/SkillTable.txt", GetLocalePathCommon());
  ...
  if (!rkNetStream.LoadInsultList(szInsultList)) {
    Tracenf("CPythonApplication - CPythonNetworkStream::LoadInsultList(%s)",
            szInsultList);
  }
...
bool CPythonApplication::Create(PyObject *poSelf, const char *c_szName,
                                int width, int height, int Windowed) {
  ...
  if (!CMSWindow::Create(c_szName, 4, 0, WindowMode,
                         ::LoadIcon(GetInstance(), MAKEINTRESOURCE(IDI_METIN2)),
                         IDC_CURSOR_NORMAL)) {
  ...
  if (!m_pySystem.IsWindowed()) {
    ...
    if (Windowed) {
      ...
      if (bAnotherWindow) {
        ...
        CMSApplication::SetPosition(GetScreenWidth() - windowWidth,
  ...
  // Device
  if (!CreateDevice(m_pySystem.GetWidth(), m_pySystem.GetHeight(), Windowed,
                    m_pySystem.GetBPP(), m_pySystem.GetFrequency()))
    return false;
  ...
  if (m_isWindowFullScreenEnable) {
    SetWindowPos(GetWindowHandle(), HWND_TOP, 0, 0, width, height,
                 SWP_SHOWWINDOW);
  }
...
// 2005.03.28 - MALL ¾ÆÀÌÅÛ¿¡ µé¾îÀÖ´Â ½Ã°£ÀÇ ´ÜÀ§°¡ ¼­¹ö¿¡¼­ time(0) À¸·Î
// ¸¸µé¾îÁö´Â
...
float CPythonApplication::GetGlobalTime() { return m_fGlobalTime; }
...
void CPythonApplication::SetCameraSpeed(int iPercentage) {
  m_fCameraRotateSpeed =
      c_fDefaultCameraRotateSpeed * float(iPercentage) / 100.0f;
  m_fCameraPitchSpeed =
      c_fDefaultCameraPitchSpeed * float(iPercentage) / 100.0f;
