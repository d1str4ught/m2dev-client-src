...
CGuildMarkImage *NewMarkImage() { return new CGuildMarkImage; }

void DeleteMarkImage(CGuildMarkImage *pkImage) { delete pkImage; }
...
void CGuildMarkImage::Destroy() { memset(&m_apxImage, 0, sizeof(m_apxImage)); }

void CGuildMarkImage::Create() { memset(m_apxImage, 0, sizeof(m_apxImage)); }
...
bool CGuildMarkImage::Load(const char *c_szFileName) {
  ...
  if (w != WIDTH || h != HEIGHT) {
    sys_err("GuildMarkImage: %s wrong dimensions (%d x %d)", c_szFileName, w,
            h);
...
void CGuildMarkImage::PutData(UINT x, UINT y, UINT width, UINT height,
                              void *data) {
...
void CGuildMarkImage::GetData(UINT x, UINT y, UINT width, UINT height,
                              void *data) {
  for (UINT row = 0; row < height; ++row) {
...
bool CGuildMarkImage::SaveMark(DWORD posMark, uint8_t *pbImage) {
  if (posMark >= MARK_TOTAL_COUNT) {
    sys_err("CGuildMarkImage::CopyMarkFromData: Invalid mark position %u",
            posMark);
    return false;
  }
  ...
  printf("PutMark pos %u %ux%u\n", posMark, colMark * SGuildMark::WIDTH,
...
// CLIENT
bool CGuildMarkImage::SaveBlockFromCompressedData(DWORD posBlock,
                                                  const uint8_t *pbComp,
                                                  DWORD dwCompSize) {
  ...
  if (LZO_E_OK != lzo1x_decompress_safe(pbComp, dwCompSize, (uint8_t *)apxBuf,
                                        (lzo_uint *)&sizeBuf,
                                        CLZO::Instance().GetWorkMemory())) {
    sys_err("CGuildMarkImage::CopyBlockFromCompressedData: cannot decompress, "
            "compressed size = %u",
            dwCompSize);
    return false;
  }
  ...
  if (sizeBuf != sizeof(apxBuf)) {
    sys_err("CGuildMarkImage::CopyBlockFromCompressedData: image corrupted, "
            "decompressed size = %u",
            sizeBuf);
    return false;
  }
  ...
  m_aakBlock[rowBlock][colBlock].CopyFrom(
      pbComp, dwCompSize,
      GetCRC32((const char *)apxBuf, sizeof(Pixel) * SGuildMarkBlock::SIZE));
  return true;
}
...
void CGuildMarkImage::GetDiffBlocks(
    const DWORD *crcList,
  ...
  for (DWORD row = 0; row < BLOCK_ROW_COUNT; ++row)
    for (DWORD col = 0; col < BLOCK_COL_COUNT; ++col) {
      if (m_aakBlock[row][col].m_crc != *crcList) {
        mapDiffBlocks.insert(
            std::map<uint8_t, const SGuildMarkBlock *>::value_type(
                posBlock, &m_aakBlock[row][col]));
      }
      ++crcList;
      ++posBlock;
    }
...
////////////////////////////////////////////////////////////////////////////////
DWORD SGuildMarkBlock::GetCRC() const { return m_crc; }

void SGuildMarkBlock::CopyFrom(const uint8_t *pbCompBuf, DWORD dwCompSize,
                               DWORD crc) {
  ...
  // SGuildMarkBlock::SIZE, m_sizeCompBuf, m_crc);
}
...
void SGuildMarkBlock::Compress(const Pixel *pxBuf) {
  ...
  if (LZO_E_OK != lzo1x_999_compress((const uint8_t *)pxBuf,
                                     ...
                                     m_abCompBuf, (lzo_uint *)&m_sizeCompBuf,
                                     CLZO::Instance().GetWorkMemory())) {
    sys_err("SGuildMarkBlock::Compress: Error! %u > %u",
            sizeof(Pixel) * SGuildMarkBlock::SIZE, m_sizeCompBuf);
    return;
  }

  // sys_log(0, "SGuildMarkBlock::Compress %u > %u", sizeof(Pixel) *
  // SGuildMarkBlock::SIZE, m_sizeCompBuf);
