...
class CPythonNetworkStream : public CNetworkStream,
                             public CSingleton<CPythonNetworkStream> {
public:
  // Table-driven packet dispatch (Phase 5)
  struct PacketHandlerEntry {
    bool (CPythonNetworkStream::*handler)();
    uint16_t minSize;
    bool exitPhase; // true = stop dispatch loop after handling (phase-changing
                    // packets)
  };
...
public:
  ...
  bool SendSyncPositionElementPacket(DWORD dwVictimVID, DWORD dwVictimX,
                                     DWORD dwVictimY);

  bool SendAttackPacket(UINT uMotAttack, DWORD dwVIDVictim);
  bool SendCharacterStatePacket(const TPixelPosition &c_rkPPosDst,
  ...
  // Fly
  bool SendFlyTargetingPacket(DWORD dwTargetVID,
                              const TPixelPosition &kPPosTarget);
  bool SendAddFlyTargetingPacket(DWORD dwTargetVID,
                                 const TPixelPosition &kPPosTarget);
  ...
  bool SendGuildChangeGradeAuthorityPacket(BYTE byGradeNumber,
                                           BYTE byAuthority);
  ...
  // Private Shop
  bool SendBuildPrivateShopPacket(
      const char *c_szName,
...
public:
  ...
  bool SendCreateCharacterPacket(BYTE index, const char *name, BYTE job,
                                 BYTE shape, BYTE byStat1, BYTE byStat2,
                                 BYTE byStat3, BYTE byStat4);
