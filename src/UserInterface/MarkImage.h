...
struct SGuildMarkBlock {
  enum {
    ...
    SIZE = WIDTH * HEIGHT,
    MAX_COMP_SIZE =
        (SIZE * sizeof(Pixel)) + ((SIZE * sizeof(Pixel)) >> 4) + 64 + 3
  };
...
public:
  enum {
    ...
    MARK_ROW_COUNT =
        BLOCK_ROW_COUNT * SGuildMarkBlock::MARK_PER_BLOCK_HEIGHT, // 40
    MARK_COL_COUNT =
        BLOCK_COL_COUNT * SGuildMarkBlock::MARK_PER_BLOCK_WIDTH, // 32
  ...
  bool SaveBlockFromCompressedData(DWORD posBlock, const uint8_t *pbComp,
                                   DWORD dwCompSize); // 서버 -> 클라이언트

  DWORD GetEmptyPosition(); // 빈 마크 위치를 얻는다.

  void GetBlockCRCList(uint32_t *crcList);
  void GetDiffBlocks(const DWORD *crcList,
