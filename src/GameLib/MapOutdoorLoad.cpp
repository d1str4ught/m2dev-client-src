...
bool CMapOutdoor::Load(float x, float y, float z) {
...

  {
    ...
    // 2004.08.09.myevan.Pack파일을 찾을때.. 폴더명만으로는 그냥 리턴되는 부분이
    // 있다
  ...
  if (!LoadSetting(strFileName.c_str()))
    TraceError("CMapOutdoor::Load : LoadSetting(%s) Failed",
               strFileName.c_str());
  ...
  // TODO: SetRenderingDevice에서 Environment로 부터 라이트 속성을 넘겨줘야
  // 스태틱 라이트가 제대로 작동한다.
  ...
  std::string local_envDataName =
      GetMapDataDirectory() + "\\" + m_settings_envDataName;
...
std::string &CMapOutdoor::GetEnvironmentDataName() { return m_envDataName; }
...
void CMapOutdoor::AssignTerrainPtr() {
  ...
  // 때문에 이 virtual method를 호출 한다. 이 메소드는 CMapOutDoor에서는 아무
  // 행동도 하지 않는다.
  ...
  sReferenceCoordMaxX = std::min(
      m_CurCoordinate.m_sTerrainCoordX + LOAD_SIZE_WIDTH, m_sTerrainCountX - 1);
  sReferenceCoordMinY =
      std::max(m_CurCoordinate.m_sTerrainCoordY - LOAD_SIZE_WIDTH, 0);
  sReferenceCoordMaxY = std::min(
      m_CurCoordinate.m_sTerrainCoordY + LOAD_SIZE_WIDTH, m_sTerrainCountY - 1);
  ...
  for (i = 0; i < m_TerrainVector.size(); ++i) {
    ...
    if (usCoordX >= sReferenceCoordMinX && usCoordX <= sReferenceCoordMaxX &&
        usCoordY >= sReferenceCoordMinY && usCoordY <= sReferenceCoordMaxY) {
      m_pTerrain
          [(usCoordY - m_CurCoordinate.m_sTerrainCoordY + LOAD_SIZE_WIDTH) * 3 +
           (usCoordX - m_CurCoordinate.m_sTerrainCoordX + LOAD_SIZE_WIDTH)] =
              pTerrain;
    }
  }
...
bool CMapOutdoor::LoadArea(WORD wAreaCoordX, WORD wAreaCoordY, WORD wCellCoordX,
                           WORD wCellCoordY) {
...
#endif
  unsigned long ulID =
      (unsigned long)(wAreaCoordX) * 1000L + (unsigned long)(wAreaCoordY);
  char szAreaPathName[64 + 1];
  _snprintf(szAreaPathName, sizeof(szAreaPathName), "%s\\%06u\\",
            GetMapDataDirectory().c_str(), ulID);
  ...
  if (!pArea->Load(szAreaPathName))
    TraceError(" CMapOutdoor::LoadArea(%d, %d) LoadShadowMap ERROR",
               wAreaCoordX, wAreaCoordY);
#ifdef _DEBUG
...
bool CMapOutdoor::LoadTerrain(WORD wTerrainCoordX, WORD wTerrainCoordY,
  ...
  unsigned long ulID =
      (unsigned long)(wTerrainCoordX) * 1000L + (unsigned long)(wTerrainCoordY);
  char filename[256];
  sprintf(filename, "%s\\%06u\\AreaProperty.txt", GetMapDataDirectory().c_str(),
          ulID);
  ...
  _snprintf(szRawHeightFieldname, sizeof(szRawHeightFieldname),
            "%s\\%06u\\height.raw", GetMapDataDirectory().c_str(), ulID);
  _snprintf(szSplatName, sizeof(szSplatName), "%s\\%06u\\tile.raw",
            GetMapDataDirectory().c_str(), ulID);
  _snprintf(szAttrMapName, sizeof(szAttrMapName), "%s\\%06u\\attr.atr",
            GetMapDataDirectory().c_str(), ulID);
  _snprintf(szWaterMapName, sizeof(szWaterMapName), "%s\\%06u\\water.wtr",
            GetMapDataDirectory().c_str(), ulID);
  _snprintf(szShadowTexName, sizeof(szShadowTexName), "%s\\%06u\\shadowmap.dds",
            GetMapDataDirectory().c_str(), ulID);
  _snprintf(szShadowMapName, sizeof(szShadowMapName), "%s\\%06u\\shadowmap.raw",
            GetMapDataDirectory().c_str(), ulID);
  _snprintf(szMiniMapTexName, sizeof(szMiniMapTexName), "%s\\%06u\\minimap.dds",
            GetMapDataDirectory().c_str(), ulID);

  if (!pTerrain->LoadWaterMap(szWaterMapName))
    TraceError(" CMapOutdoor::LoadTerrain(%d, %d) LoadWaterMap ERROR",
               wTerrainCoordX, wTerrainCoordY);

  if (!pTerrain->LoadHeightMap(szRawHeightFieldname))
    TraceError(" CMapOutdoor::LoadTerrain(%d, %d) LoadHeightMap ERROR",
               wTerrainCoordX, wTerrainCoordY);

  if (!pTerrain->LoadAttrMap(szAttrMapName))
    TraceError(" CMapOutdoor::LoadTerrain(%d, %d) LoadAttrMap ERROR",
               wTerrainCoordX, wTerrainCoordY);

  if (!pTerrain->RAW_LoadTileMap(szSplatName))
    TraceError(" CMapOutdoor::LoadTerrain(%d, %d) RAW_LoadTileMap ERROR",
               wTerrainCoordX, wTerrainCoordY);

  pTerrain->LoadShadowTexture(szShadowTexName);

  if (!pTerrain->LoadShadowMap(szShadowMapName))
    TraceError(" CMapOutdoor::LoadTerrain(%d, %d) LoadShadowMap ERROR",
               wTerrainCoordX, wTerrainCoordY);
...
bool CMapOutdoor::LoadSetting(const char *c_szFileName) {
  ...
  if (!LoadMultipleTextData(c_szFileName, stTokenVectorMap)) {
    TraceError(
        "MapOutdoor::LoadSetting(c_szFileName=%s) - LoadMultipleTextData",
        c_szFileName);
    return false;
  }
  ...
  if (stTokenVectorMap.end() == stTokenVectorMap.find("scripttype")) {
    TraceError(
        "MapOutdoor::LoadSetting(c_szFileName=%s) - FIND 'scripttype' - FAILED",
        c_szFileName);
    return false;
  }
  ...
  if (stTokenVectorMap.end() == stTokenVectorMap.find("viewradius")) {
    TraceError(
        "MapOutdoor::LoadSetting(c_szFileName=%s) - FIND 'viewradius' - FAILED",
        c_szFileName);
    return false;
  }
  ...
  if (stTokenVectorMap.end() == stTokenVectorMap.find("cellscale")) {
    TraceError(
        "MapOutdoor::LoadSetting(c_szFileName=%s) - FIND 'cellscale' - FAILED",
        c_szFileName);
    return false;
  }

  if (stTokenVectorMap.end() == stTokenVectorMap.find("heightscale")) {
    TraceError("MapOutdoor::LoadSetting(c_szFileName=%s) - FIND 'heightscale' "
               "- FAILED",
               c_szFileName);
    return false;
  }
  ...
  if (stTokenVectorMap.end() == stTokenVectorMap.find("mapsize")) {
    TraceError(
        "MapOutdoor::LoadSetting(c_szFileName=%s) - FIND 'mapsize' - FAILED",
        c_szFileName);
    return false;
  }
  ...
  if (stTokenVectorMap.end() == stTokenVectorMap.find("textureset")) {
    TraceError(
        "MapOutdoor::LoadSetting(c_szFileName=%s) - FIND 'textureset' - FAILED",
        c_szFileName);
    return false;
  }

  if (stTokenVectorMap.end() != stTokenVectorMap.find("terrainvisible")) {
    m_bSettingTerrainVisible =
        (bool)(atoi(stTokenVectorMap["terrainvisible"][0].c_str()) != 0);
  } else {
  ...
  if (c_rstrType != "MapSetting") {
    TraceError("MapOutdoor::LoadSetting(c_szFileName=%s) - Resourse Type ERROR",
               c_szFileName);
    return false;
  }
  ...
  if (0L >= m_lViewRadius) {
    TraceError("MapOutdoor::LoadSetting(c_szFileName=%s) - VIEWRADIUS IS NOT "
               "GREATER THAN 0",
               c_szFileName);
    return false;
  }
  ...
  m_fTerrainTexCoordBase =
      1.0f / (float)(CTerrainImpl::PATCH_XSIZE * CTerrainImpl::CELLSCALE);
  ...
  if (stTokenVectorMap.end() != stTokenVectorMap.find("baseposition")) {
    ...
    SetBaseXY((DWORD)atol(c_rstrMapBaseX.c_str()),
              (DWORD)atol(c_rstrMapBaseY.c_str()));
  }
  ...
  if (!m_TextureSet.Load(stTextureSetFileName.c_str(),
                         m_fTerrainTexCoordBase)) {
    TraceError(
        "MapOutdoor::LoadSetting(c_szFileName=%s) - LOAD TEXTURE SET(%s) ERROR",
        c_szFileName, stTextureSetFileName.c_str());
    return false;
  }
  ...
  if (stTokenVectorMap.end() != stTokenVectorMap.find("environment")) {
    ...
    if (!c_rEnvironmentVector.empty())
      ...
      TraceError("CMapOutdoor::LoadSetting(c_szFileName=%s) - Failed to load "
                 "environment data\n",
                 c_szFileName);
  }
  ...
  D3DXMatrixScaling(
      &m_matStaticShadow,
      +m_fTerrainTexCoordBase * ((float)CTerrainImpl::PATCH_XSIZE /
                                 static_cast<float>(CTerrainImpl::XSIZE)),
      -m_fTerrainTexCoordBase * ((float)CTerrainImpl::PATCH_YSIZE /
                                 static_cast<float>(CTerrainImpl::XSIZE)),
      0.0f);
  m_matStaticShadow._41 = 0.0f;
  m_matStaticShadow._42 = 0.0f;

  D3DXMatrixScaling(&m_matDynamicShadowScale, 1.0f / 2550.0f, -1.0f / 2550.0f,
                    1.0f);
  m_matDynamicShadowScale._41 = 0.5f;
  m_matDynamicShadowScale._42 = 0.5f;

  // Transform
  D3DXMatrixScaling(&m_matBuildingTransparent, 1.0f / ((float)ms_iWidth),
                    -1.0f / ((float)ms_iHeight), 1.0f);
...
bool CMapOutdoor::LoadMonsterAreaInfo() {
  ...
  if (!CPackManager::Instance().GetFile(c_szFileName, File)) {
    // TraceError(" CMapOutdoorAccessor::LoadMonsterAreaInfo Load File %s
    // ERROR", c_szFileName);
    return false;
  }
  ...
  for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i) {
    ...
    if (0 == stTokenVector[0].compare("m") ||
        0 == stTokenVector[0].compare("g")) {
      if (stTokenVector.size() < 11) {
        TraceError("CMapOutdoorAccessor::LoadMonsterAreaInfo Get MonsterInfo "
                   "File Format ERROR! continue....");
        continue;
      }
      ...
      CMonsterAreaInfo *pMonsterAreaInfo =
          AddMonsterAreaInfo(lOriginX, lOriginY, lSizeX, lSizeY);
      ...
      else if (CMonsterAreaInfo::MONSTERAREAINFOTYPE_GROUP ==
               eMonsterAreaInfoType)
