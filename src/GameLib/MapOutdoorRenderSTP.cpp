...
void CMapOutdoor::__RenderTerrain_RenderSoftwareTransformPatch() {
  ...
  std::vector<std::pair<float, long>>::iterator far_it =
      std::upper_bound(m_PatchVector.begin(), m_PatchVector.end(), fog_far);
  std::vector<std::pair<float, long>>::iterator near_it =
  ...
  STATEMANAGER.SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_SPECULAR |
                      D3DFVF_TEX2);
  ...
  for (; it != near_it; ++it) {
    ...
    __SoftwareTransformPatch_RenderPatchSplat(
        kTPRS, it->second, wPrimitiveCount, ePrimitiveType, false);
  ...
  if (m_iRenderedSplatNum < m_iSplatLimit) {
    for (it = near_it; it != far_it; ++it) {
      ...
      __SoftwareTransformPatch_RenderPatchSplat(
          kTPRS, it->second, wPrimitiveCount, ePrimitiveType, true);
  ...
  if (IsFastTNL()) {
    ...
    if (m_iRenderedSplatNum < m_iSplatLimit) {
      for (it = far_it; it != m_PatchVector.end(); ++it) {
        __SoftwareTransformPatch_RenderPatchNone(
            kTPRS, it->second, wPrimitiveCount, ePrimitiveType);
...
void CMapOutdoor::__SoftwareTransformPatch_RenderPatchSplat(
  ...
  SoftwareTransformPatch_STLVertex
      akTransVertex[CTerrainPatch::TERRAIN_VERTEX_COUNT];
  if (!__SoftwareTransformPatch_SetTransform(
          rkTPRS, akTransVertex, *pTerrainPatchProxy, wCoordX, wCoordY,
          isFogEnable, isDynamicShadow))
    return;
  ...
  for (DWORD j = 1; j < pTerrain->GetNumTextures(); ++j) {
    ...
    std::vector<int>::iterator aIterator =
        std::find(m_RenderedTextureNumVector.begin(),
...
void CMapOutdoor::__SoftwareTransformPatch_RenderPatchNone(
    SoftwareTransformPatch_SRenderState &rkTPRS, long patchnum,
  ...
  SoftwareTransformPatch_SSourceVertex *akSrcVertex =
      pTerrainPatchProxy->SoftwareTransformPatch_GetTerrainVertexDataPtr();
  if (!akSrcVertex)
    return;
  ...
  SoftwareTransformPatch_STVertex
      akTransVertex[CTerrainPatch::TERRAIN_VERTEX_COUNT];

  D3DXVECTOR4 *akPosition = (D3DXVECTOR4 *)akTransVertex;
  D3DXVECTOR4 *pkPosition;
  for (UINT uIndex = 0; uIndex != CTerrainPatch::TERRAIN_VERTEX_COUNT;
       ++uIndex) {
  ...
  DWORD dwVBSize = sizeof(SoftwareTransformPatch_STVertex) *
  ...
  STATEMANAGER.SetStreamSource(0, pkVB,
                               sizeof(SoftwareTransformPatch_STVertex));
  STATEMANAGER.DrawIndexedPrimitive(
      ePrimitiveType, 0, m_iPatchTerrainVertexCount, 0, wPrimitiveCount);
  ms_faceCount += wPrimitiveCount;
}
...
void CMapOutdoor::__SoftwareTransformPatch_RestoreRenderState(
    DWORD dwFogEnable) {}

void CMapOutdoor::__SoftwareTransformPatch_BuildPipeline(
  ...
  D3DXMATRIX m4View;
  STATEMANAGER.GetTransform(D3DTS_VIEW, &m4View);
  D3DXMATRIX m4Proj;
  STATEMANAGER.GetTransform(D3DTS_PROJECTION, &m4Proj);
...
bool CMapOutdoor::__SoftwareTransformPatch_SetTransform(
    SoftwareTransformPatch_SRenderState &rkTPRS,
  ...
  SoftwareTransformPatch_SSourceVertex *akSrcVertex =
      rkTerrainPatchProxy.SoftwareTransformPatch_GetTerrainVertexDataPtr();
  if (!akSrcVertex)
    return false;

  rkTerrainPatchProxy.SoftwareTransformPatch_UpdateTerrainLighting(
      m_kSTPD.m_dwLightVersion, rkTPRS.m_kLight, rkTPRS.m_kMtrl);
  ...
  float fTerrainBaseX = -(float)(uTerrainX * CTerrainImpl::TERRAIN_XSIZE) +
                        m_fTerrainTexCoordBase * 12.30769f;
  float fTerrainBaseY = +(float)(uTerrainY * CTerrainImpl::TERRAIN_YSIZE) +
                        m_fTerrainTexCoordBase * 12.30769f;
  ...
  if (isFogEnable) {
    ...
    for (UINT uIndex = 0; uIndex != CTerrainPatch::TERRAIN_VERTEX_COUNT;
      ...
      kWorkVertex.kPosition.x =
          (kWorkVertex.kPosition.x + 1.0f) * fScreenHalfWidth;
      kWorkVertex.kPosition.y =
          (kWorkVertex.kPosition.y - 1.0f) * fScreenHalfHeight;
      ...
      if (fFogCur < 0.0f)
        kWorkVertex.dwFog = kWorkVertex.dwDiffuse =
            0x0000000 | (kWorkVertex.dwDiffuse & 0xffffff);
      else if (fFogCur > 1.0f)
        kWorkVertex.dwFog = kWorkVertex.dwDiffuse =
            0xFF000000 | (kWorkVertex.dwDiffuse & 0xffffff);
      else
        kWorkVertex.dwFog = kWorkVertex.dwDiffuse =
            BYTE(255.0f * fFogCur) << 24 | (kWorkVertex.dwDiffuse & 0xffffff);
  ...
  if (isDynamicShadow) {
    ...
    for (UINT uIndex = 0; uIndex != CTerrainPatch::TERRAIN_VERTEX_COUNT;
         ++uIndex) {
      D3DXVec3TransformCoord(&v3Shadow, &akSrcVertex[uIndex].kPosition,
                             &m4DynamicShadow);
...
bool CMapOutdoor::__SoftwareTransformPatch_SetSplatStream(
    SoftwareTransformPatch_STLVertex *akSrcVertex) {
  ...
  DWORD dwVBSize = sizeof(SoftwareTransformPatch_SSplatVertex) *
                   CTerrainPatch::TERRAIN_VERTEX_COUNT;
  ...
  for (UINT uIndex = 0; uIndex != CTerrainPatch::TERRAIN_VERTEX_COUNT; ++uIndex)
    *(akDstVertex + uIndex) =
        *((SoftwareTransformPatch_SSplatVertex *)(akSrcVertex + uIndex));

  pkVB->Unlock();

  STATEMANAGER.SetStreamSource(0, pkVB,
                               sizeof(SoftwareTransformPatch_SSplatVertex));
  return true;
}

bool CMapOutdoor::__SoftwareTransformPatch_SetShadowStream(
    SoftwareTransformPatch_STLVertex *akSrcVertex) {
  IDirect3DVertexBuffer9 *pkVB = m_kSTPD.m_pkVBSplat[m_kSTPD.m_dwSplatPos++];
  m_kSTPD.m_dwSplatPos %= SoftwareTransformPatch_SData::SPLAT_VB_NUM;
  if (!pkVB)
  ...
  // ms_lpd3dDevice->SetStreamSource(0, pkVB,
  // sizeof(SoftwareTransformPatch_SSplatVertex));
  return true;
}
...
void CMapOutdoor::__SoftwareTransformPatch_Initialize() {
  {
    for (UINT uIndex = 0; uIndex != SoftwareTransformPatch_SData::SPLAT_VB_NUM;
         ++uIndex)
  ...
  {
    for (UINT uIndex = 0; uIndex != SoftwareTransformPatch_SData::NONE_VB_NUM;
         ++uIndex)
...
bool CMapOutdoor::__SoftwareTransformPatch_Create() {
  {
    for (UINT uIndex = 0; uIndex != SoftwareTransformPatch_SData::SPLAT_VB_NUM;
      ...
      if (FAILED(ms_lpd3dDevice->CreateVertexBuffer(
              ...
              D3DPOOL_DEFAULT, &m_kSTPD.m_pkVBSplat[uIndex], nullptr)))
        return false;
    }
  }

  {
    for (UINT uIndex = 0; uIndex != SoftwareTransformPatch_SData::NONE_VB_NUM;
      ...
      if (FAILED(ms_lpd3dDevice->CreateVertexBuffer(
              sizeof(SoftwareTransformPatch_STVertex) *
                  CTerrainPatch::TERRAIN_VERTEX_COUNT,
              D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW,
              D3DPOOL_DEFAULT, &m_kSTPD.m_pkVBNone[uIndex], nullptr)))
        return false;
    }
  }
  return true;
