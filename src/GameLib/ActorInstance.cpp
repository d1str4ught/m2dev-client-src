...
IBackground &CActorInstance::GetBackground() { return IBackground::Instance(); }

void CActorInstance::SetMainInstance() { m_isMain = true; }

void CActorInstance::SetParalysis(bool isParalysis) {
  m_isParalysis = isParalysis;
}

void CActorInstance::SetFaint(bool isFaint) { m_isFaint = isFaint; }
...
void CActorInstance::SetReachScale(float fScale) { m_fReachScale = fScale; }

float CActorInstance::__GetReachScale() { return m_fReachScale; }

float CActorInstance::__GetAttackSpeed() { return m_fAtkSpd; }
...
void CActorInstance::SetAttackSpeed(float fAtkSpd) { m_fAtkSpd = fAtkSpd; }
...
void CActorInstance::SetActorType(UINT eType) { m_eActorType = eType; }

UINT CActorInstance::GetActorType() const { return m_eActorType; }
...
BOOL CActorInstance::IsSleep() { return m_isSleep; }

BOOL CActorInstance::IsParalysis() { return m_isParalysis; }

BOOL CActorInstance::IsFaint() { return m_isFaint; }

BOOL CActorInstance::IsResistFallen() { return m_isResistFallen; }

BOOL CActorInstance::IsMoving() { return __IsMoveMotion(); }

BOOL CActorInstance::IsWaiting() { return __IsWaitMotion(); }

BOOL CActorInstance::IsDead() { return m_isRealDead; }

BOOL CActorInstance::IsKnockDown() { return __IsKnockDownMotion(); }

BOOL CActorInstance::IsDamage() { return __IsDamageMotion(); }
...
void CActorInstance::PhysicsProcess() {
  ...
  AddMovement(m_PhysicsObject.GetXMovement(), m_PhysicsObject.GetYMovement(),
              0.0f);
}

void CActorInstance::__AccumulationMovement(float fRot) {
  // NOTE - 일단은 WAIT로 미끄러짐 방지
  //        추후에는 RaceMotionData가 이동되는 모션인지에 대한 Flag를 갖고
  //        있게끔 한다. - [levites]
...
void CActorInstance::OnUpdateCollisionData(
...
void CActorInstance::OnUpdateHeighInstance(
...
bool CActorInstance::OnGetObjectHeight(float fX, float fY, float *pfHeight) {
  ...
  return m_pHeightAttributeInstance->GetHeight(fX, fY, pfHeight) == 1 ? true
                                                                      : false;
}
...
BOOL CActorInstance::IsStun() { return m_isStun; }

void CActorInstance::Stun() { m_isStun = TRUE; }
...
void CActorInstance::AdjustDynamicCollisionMovement(
    const CActorInstance *c_pActorInstance) {
  if (m_pkHorse) {
    m_pkHorse->AdjustDynamicCollisionMovement(c_pActorInstance);
    return;
  }

  // NOTE : 기존의 Sphere Overlap됬을경우 처리가 비비기를 하면은 Penetration될
  // 위험이 많아서 ( 실제로도 나왔고 --) Sphere간 Collision이 생겼을 경우
  // 이전위치로 RollBack하는 방식으로 바꿨다. 단 BGObject에 대해서만.

  if (isAttacking())
    return;

  UINT uActorType = c_pActorInstance->GetActorType();
  if ((uActorType == TYPE_BUILDING) || (uActorType == TYPE_OBJECT) ||
      (uActorType == TYPE_DOOR) || (uActorType == TYPE_STONE)) {
    BlockMovement();

    // Movement초기화
    /*	m_v3Movement = D3DXVECTOR3(0.f,0.f,0.f);

            TCollisionPointInstanceListIterator itMain =
       m_BodyPointInstanceList.begin(); for (; itMain !=
       m_BodyPointInstanceList.end(); ++itMain)
            {
                    CDynamicSphereInstanceVector & c_rMainSphereVector =
       (*itMain).SphereInstanceVector; for (DWORD i = 0; i <
       c_rMainSphereVector.size(); ++i)
                    {
                            CDynamicSphereInstance & c_rMainSphere =
       c_rMainSphereVector[i]; c_rMainSphere.v3Position
       =c_rMainSphere.v3LastPosition;
                    }
            }*/
  } else {
    ...
    for (; itMain != m_BodyPointInstanceList.end(); ++itMain) {
      CDynamicSphereInstanceVector &c_rMainSphereVector =
          (*itMain).SphereInstanceVector;
      for (DWORD i = 0; i < c_rMainSphereVector.size(); ++i) {
        CDynamicSphereInstance &c_rMainSphere = c_rMainSphereVector[i];

        TCollisionPointInstanceList::const_iterator itOpp =
            c_pActorInstance->m_BodyPointInstanceList.begin();
        for (; itOpp != c_pActorInstance->m_BodyPointInstanceList.end();
             ++itOpp) {
          CSphereCollisionInstance s;
          s.GetAttribute().fRadius = itOpp->SphereInstanceVector[0].fRadius;
          s.GetAttribute().v3Position =
              itOpp->SphereInstanceVector[0].v3Position;
...
void CActorInstance::UpdateSplashArea() {
  const CRaceMotionData::TMotionAttackingEventData *c_pAttackingData =
      m_kSplashArea.c_pAttackingEvent;
  if (!c_pAttackingData)
    return;

  if (m_kSplashArea.SphereInstanceVector.size() !=
      c_pAttackingData->CollisionData.SphereDataVector.size())
    return;

  float fRadian = D3DXToRadian(270.0f + 360.0f - GetRotation());

  for (DWORD i = 0; i < c_pAttackingData->CollisionData.SphereDataVector.size();
       ++i) {
    const TSphereData &c_rSphereData =
        c_pAttackingData->CollisionData.SphereDataVector[i].GetAttribute();
    CDynamicSphereInstance &rSphereInstance =
        m_kSplashArea.SphereInstanceVector[i];

    rSphereInstance.v3Position.x = m_x +
                                   c_rSphereData.v3Position.x * sinf(fRadian) +
                                   c_rSphereData.v3Position.y * cosf(fRadian);

    rSphereInstance.v3Position.y = m_y +
                                   c_rSphereData.v3Position.x * cosf(fRadian) -
...
void CActorInstance::__AdjustCollisionMovement(
    const CGraphicObjectInstance *c_pGraphicObjectInstance) {
  if (m_pkHorse) {
  ...
  for (; itMain != m_BodyPointInstanceList.end(); ++itMain) {
    ...
    for (DWORD i = 0; i < c_rMainSphereVector.size(); ++i) {
      ...
      D3DXVECTOR3 v3Delta =
          c_pGraphicObjectInstance->GetCollisionMovementAdjust(c_rMainSphere);
      m_v3Movement += v3Delta;
      c_rMainSphere.v3Position += v3Delta;
      ...
      {
              const D3DXVECTOR3 & c_rv3Position =
      c_pObjectInstance->GetPosition();
...
float CActorInstance::GetHeight() { return CGraphicThingInstance::GetHeight(); }

bool CActorInstance::IntersectDefendingSphere() {
  for (TCollisionPointInstanceList::iterator it =
           m_DefendingPointInstanceList.begin();
       it != m_DefendingPointInstanceList.end(); ++it) {
    CDynamicSphereInstanceVector &rSphereInstanceVector =
        (*it).SphereInstanceVector;
...
bool CActorInstance::__IsMountingHorse() { return NULL != m_pkHorse; }
...
void CActorInstance::__CreateTree(const char *c_szFileName) {
  ...
  m_pkTree = rkForest.CreateInstance(
      m_x, m_y, m_z, GetCaseCRC32(c_szFileName, strlen(c_szFileName)),
      c_szFileName);
...
void CActorInstance::ClearAttachingEffect() { __ClearAttachingEffect(); }
...
CActorInstance::~CActorInstance() { Destroy(); }
