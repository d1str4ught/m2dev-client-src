...
CArea *CArea::New() { return ms_kPool.Alloc(); }
...
void CArea::Update() { __UpdateAniThingList(); }

void CArea::UpdateAroundAmbience(float fX, float fY, float fZ) {
  // Ambience
  TAmbienceInstanceVector::iterator i;
  for (i = m_AmbienceCloneInstanceVector.begin();
       i != m_AmbienceCloneInstanceVector.end(); ++i) {
...
struct CArea_FEffectInstanceRender {
  inline void operator()(CEffectInstance *pkEftInst) { pkEftInst->Render(); }
};
...
void CArea::RenderEffect() {
  ...
  if (m_isDisableSortRendering) {
    ...
    std::sort(s_kVct_pkEftInstSort.begin(), s_kVct_pkEftInstSort.end(),
              CArea_LessEffectInstancePtrRenderOrder());
    std::for_each(s_kVct_pkEftInstSort.begin(), s_kVct_pkEftInstSort.end(),
                  CArea_FEffectInstanceRender());
  }
}
...
void CArea::CollectRenderingObject(
    std::vector<CGraphicThingInstance *> &rkVct_pkOpaqueThingInst) {
  TThingInstanceVector::iterator i;
  for (i = m_ThingCloneInstaceVector.begin();
       i != m_ThingCloneInstaceVector.end(); ++i) {
...
void CArea::CollectBlendRenderingObject(
    std::vector<CGraphicThingInstance *> &rkVct_pkBlendThingInst) {
  TThingInstanceVector::iterator i;
  for (i = m_ThingCloneInstaceVector.begin();
       i != m_ThingCloneInstaceVector.end(); ++i) {
    CGraphicThingInstance *pkThingInst = *i;
...
void CArea::Render() {
  ...
  while (i != m_ThingCloneInstaceVector.end()) {
    ...
    if (pkThingInst->Render()) {
      aGraphicThingInstanceCRCMapIterator =
          m_GraphicThingInstanceCRCMap.find(pkThingInst);
      DWORD dwCRC = (*aGraphicThingInstanceCRCMapIterator).second;

      m_kRenderedGrapphicThingInstanceVector.push_back(pkThingInst);

      TCRCWithNumberVector::iterator aCRCWithNumberVectorIterator =
          std::find_if(m_kRenderedThingInstanceCRCWithNumberVector.begin(),
                       m_kRenderedThingInstanceCRCWithNumberVector.end(),
                       FFindIfCRC(dwCRC));

      if (m_kRenderedThingInstanceCRCWithNumberVector.end() ==
          aCRCWithNumberVectorIterator) {
  ...
  std::sort(m_kRenderedThingInstanceCRCWithNumberVector.begin(),
            m_kRenderedThingInstanceCRCWithNumberVector.end(), CRCNumComp());
}
...
void CArea::RenderAmbience() {
  ...
  STATEMANAGER.SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_SELECTARG1);
  TAmbienceInstanceVector::iterator itor =
      m_AmbienceCloneInstanceVector.begin();
...
void CArea::RenderDungeon() {
  ...
  TDungeonBlockInstanceVector::iterator itor =
      m_DungeonBlockCloneInstanceVector.begin();
...
void CArea::Refresh() {
  ...
  for (it = m_ObjectInstanceVector.begin(); it != m_ObjectInstanceVector.end();
       ++it) {
    ...
    if (prt::PROPERTY_TYPE_TREE == pObjectInstance->dwType) {
      ...
      if (pObjectInstance->pThingInstance->IsMotionThing()) {
        m_AniThingCloneInstanceVector.push_back(
            pObjectInstance->pThingInstance);
        pObjectInstance->pThingInstance->SetMotion(0);
      }

      if (pObjectInstance->isShadowFlag) {
        m_ShadowThingCloneInstaceVector.push_back(
            pObjectInstance->pThingInstance);
      }

      if (pObjectInstance->pAttributeInstance) {
        pObjectInstance->pThingInstance->UpdateCollisionData(
            &pObjectInstance->pAttributeInstance->GetObjectPointer()
                 ->GetCollisionDataVector());
        pObjectInstance->pAttributeInstance->RefreshObject(
            pObjectInstance->pThingInstance->GetTransform());
        pObjectInstance->pThingInstance->UpdateHeightInstance(
            pObjectInstance->pAttributeInstance);
      }
    } else if (prt::PROPERTY_TYPE_EFFECT == pObjectInstance->dwType) {
    } else if (prt::PROPERTY_TYPE_AMBIENCE == pObjectInstance->dwType) {
      m_AmbienceCloneInstanceVector.push_back(
          pObjectInstance->pAmbienceInstance);
      ...
      m_DungeonBlockCloneInstanceVector.push_back(
          pObjectInstance->pDungeonBlock);
      ...
      if (pObjectInstance->pAttributeInstance) {
        pObjectInstance->pDungeonBlock->UpdateCollisionData(
            &pObjectInstance->pAttributeInstance->GetObjectPointer()
                 ->GetCollisionDataVector());
        pObjectInstance->pAttributeInstance->RefreshObject(
            pObjectInstance->pDungeonBlock->GetTransform());
        pObjectInstance->pDungeonBlock->UpdateHeightInstance(
            pObjectInstance->pAttributeInstance);
      }
    }
  }
...
void CArea::__Load_BuildObjectInstances() {
  ...
  std::sort(m_ObjectDataVector.begin(), m_ObjectDataVector.end(),
            ObjectDataComp());
  ...
  for (it = m_ObjectInstanceVector.begin(); it != m_ObjectInstanceVector.end();
    ...
    *it = ms_ObjectInstancePool.Alloc();
    ...
    if ((*it)->dwType == prt::PROPERTY_TYPE_BUILDING)
      m_GraphicThingInstanceCRCMap.insert(
          TGraphicThingInstanceCRCMap::value_type((*it)->pThingInstance,
                                                  c_pObjectData->dwCRC));
  }
...
void CArea::__SetObjectInstance(TObjectInstance *pObjectInstance,
                                const TObjectData *c_pData) {
  CProperty *pProperty;
...
void CArea::__SetObjectInstance_SetEffect(TObjectInstance *pObjectInstance,
                                          const TObjectData *c_pData,
                                          CProperty *pProperty) {
  ...
  pObjectInstance->dwEffectID =
      GetCaseCRC32(Data.strFileName.c_str(), Data.strFileName.size());
  ...
  D3DXMATRIX mat;
  D3DXMatrixRotationYawPitchRoll(&mat, D3DXToRadian(c_pData->m_fYaw),
  ...
  m_EffectInstanceMap.insert(TEffectInstanceMap::value_type(
      pObjectInstance->dwEffectInstanceIndex, pEffectInstance));
}

void CArea::__SetObjectInstance_SetTree(TObjectInstance *pObjectInstance,
                                        const TObjectData *c_pData,
                                        CProperty *pProperty) {
  ...
  pObjectInstance->SetTree(c_pData->Position.x, c_pData->Position.y,
                           c_pData->Position.z + c_pData->m_fHeightBias,
                           c_pData->dwCRC, c_szTreeName);
}

void CArea::TObjectInstance::SetTree(float x, float y, float z, DWORD dwTreeCRC,
                                     const char *c_szTreeName) {
...
void CArea::__SetObjectInstance_SetBuilding(TObjectInstance *pObjectInstance,
                                            const TObjectData *c_pData,
                                            CProperty *pProperty) {
  ...
  CGraphicThing *pThing =
      (CGraphicThing *)rkResMgr.GetResourcePointer(Data.strFileName.c_str());
  pThing->AddReference();

  if (pThing->IsEmpty()) {
#ifdef _DEBUG
    TraceError("CArea::SetBuilding: There is no data: %s",
               Data.strFileName.c_str());
#endif
    return;
  }
...

  {
    ...
    for (UINT uLODIndex = 1; uLODIndex <= 3; ++uLODIndex) {
      ...
      stLODModelFileName = CFileNameHelper::NoExtension(stSrcModelFileName) +
                           szLODModelFileNameEnd;
      if (!rkResMgr.IsFileExist(stLODModelFileName.c_str()))
        break;

      CGraphicThing *pLODModelThing =
          (CGraphicThing *)rkResMgr.GetResourcePointer(
              stLODModelFileName.c_str());
      if (!pLODModelThing)
        break;
  ...
  pObjectInstance->pThingInstance->SetPosition(
      c_pData->Position.x, c_pData->Position.y,
      c_pData->Position.z + c_pData->m_fHeightBias);
  pObjectInstance->pThingInstance->SetRotation(
      c_pData->m_fYaw, c_pData->m_fPitch, c_pData->m_fRoll);
...
void CArea::__SetObjectInstance_SetAmbience(TObjectInstance *pObjectInstance,
                                            const TObjectData *c_pData,
                                            CProperty *pProperty) {
  pObjectInstance->pAmbienceInstance = ms_AmbienceInstancePool.Alloc();
  if (!prt::PropertyAmbienceStringToData(
          pProperty, &pObjectInstance->pAmbienceInstance->AmbienceData))
    return;
  ...
  pAmbienceInstance->fMaxVolumeAreaPercentage =
      c_pData->fMaxVolumeAreaPercentage;
...
void CArea::__SetObjectInstance_SetDungeonBlock(
    TObjectInstance *pObjectInstance, const TObjectData *c_pData,
    CProperty *pProperty) {
  ...
  pObjectInstance->pDungeonBlock->SetPosition(
      c_pData->Position.x, c_pData->Position.y,
      c_pData->Position.z + c_pData->m_fHeightBias);
...
void CArea::__LoadAttribute(TObjectInstance *pObjectInstance,
                            const char *c_szAttributeFileName) {
  // OBB를 사용한 충돌 정보 자동 생성.
  const bool bFileExist =
      CResourceManager::Instance().IsFileExist(c_szAttributeFileName);

  CAttributeData *pAttributeData =
      (CAttributeData *)CResourceManager::Instance().GetResourcePointer(
          c_szAttributeFileName);
  ...
  if (false == bFileExist) {
    ...
    std::transform(attrFileName.begin(), attrFileName.end(),
                   attrFileName.begin(),
    ...
    // NOTE: dungeon 오브젝트는 Dummy Collision을 자동으로 생성하지 않도록 함
    // (던전의 경우 더미 컬리전때문에 문제가 된 경우가 수차례 있었음. 이렇게
    // 하기로 그래픽 팀과 협의 완료)
    if (pAttributeData->IsEmpty() && false == bIsDungeonObject) {
      if (NULL != pObjectInstance && NULL != pObjectInstance->pThingInstance) {
        ...
        D3DXQuaternionRotationYawPitchRoll(&collision.quatRotation,
...
bool CArea::Load(const char *c_szPathName) {
  ...
  std::string strObjectDataFileName =
      c_szPathName + std::string("AreaData.txt");
  std::string strAmbienceDataFileName =
      c_szPathName + std::string("AreaAmbienceData.txt");
...
bool CArea::__Load_LoadObject(const char *c_szFileName) {
  ...
  for (DWORD i = 0; i < dwCount; ++i) {
    ...
    ObjectData.InitializeRotation(); // ObjectData.m_fYaw = ObjectData.m_fPitch
                                     // = ObjectData.m_fRoll = 0;
    if (rVector.size() > 4) {
    ...
    if (rVector.size() > 6) {
      for (int portalIdx = 0;
           portalIdx < std::min(rVector.size() - 6, (size_t)PORTAL_ID_MAX_NUM);
           ++portalIdx) {
        ObjectData.abyPortalID[portalIdx] =
            atoi(rVector[6 + portalIdx].c_str());
      }
    }
    ...
    if (!CPropertyManager::Instance().Get(ObjectData.dwCRC, &pProperty)) {
      TraceError(" CArea::LoadObject Property(%u) Load ERROR",
                 ObjectData.dwCRC);
      continue;
    }
...
bool CArea::__Load_LoadAmbience(const char *c_szFileName) {
  ...
  for (DWORD i = 0; i < dwCount; ++i) {
    ...
    CProperty *pProperty;
    if (!CPropertyManager::Instance().Get(ObjectData.dwCRC, &pProperty)) {
      TraceError(" CArea::LoadAmbience Property(%d) Load ERROR",
                 ObjectData.dwCRC);
      continue;
    }
...
DWORD CArea::GetObjectDataCount() { return m_ObjectDataVector.size(); }

bool CArea::GetObjectDataPointer(DWORD dwIndex,
                                 const TObjectData **ppObjectData) const {
...
const bool CArea::GetObjectInstancePointer(
    const DWORD &dwIndex, const TObjectInstance **ppObjectInstance) const {
...
void CArea::ClearPortal() { m_kSet_ShowingPortalID.clear(); }
...
void CArea::RefreshPortal() {
  ...
  for (DWORD i = 0; i < m_ObjectDataVector.size(); ++i) {
    ...
    for (int j = 0; j < PORTAL_ID_MAX_NUM; ++j) {
      ...
      if (m_kSet_ShowingPortalID.end() ==
          m_kSet_ShowingPortalID.find(byPortalID))
        continue;
  ...
  for (it = m_ObjectInstanceVector.begin(); it != m_ObjectInstanceVector.end();
    ...
    if (m_bPortalEnable) {
      if (kSet_ShowingObjectInstance.end() ==
          kSet_ShowingObjectInstance.find(pObjectInstance))
        continue;
    }
...
void CArea::__Clear_DestroyObjectInstance(TObjectInstance *pObjectInstance) {
  if (pObjectInstance->dwEffectInstanceIndex != 0xffffffff) {
    TEffectInstanceIterator f =
        m_EffectInstanceMap.find(pObjectInstance->dwEffectInstanceIndex);
...
void CArea::SetCoordinate(const unsigned short &usCoordX,
                          const unsigned short &usCoordY) {
  m_wX = usCoordX;
  m_wY = usCoordY;
}
...
void CArea::TAmbienceInstance::__Update(float fxCenter, float fyCenter,
                                        float fzCenter) {
  if (0 == dwRange)
    return;
...
void CArea::TAmbienceInstance::UpdateOnceSound(float fxCenter, float fyCenter,
                                               float fzCenter) {
  float fDistance = sqrtf((fx - fxCenter) * (fx - fxCenter) +
                          (fy - fyCenter) * (fy - fyCenter) +
  ...
  if (uint32_t(fDistance) < dwRange) {
    if (!playSoundInstance) {
      ...
      playSoundInstance =
          SoundEngine::Instance().PlayAmbienceSound3D(fx, fy, fz, c_szFileName);
    }
  } else {
...
void CArea::TAmbienceInstance::UpdateStepSound(float fxCenter, float fyCenter,
                                               float fzCenter) {
  float fDistance = sqrtf((fx - fxCenter) * (fx - fxCenter) +
                          (fy - fyCenter) * (fy - fyCenter) +
                          (fz - fzCenter) * (fz - fzCenter));
  if (DWORD(fDistance) < dwRange) {
    ...
    if (fcurTime > fNextPlayTime) {
      ...
      fNextPlayTime += AmbienceData.fPlayInterval +
                       frandom(0.0f, AmbienceData.fPlayIntervalVariation);
    }
  } else {
    playSoundInstance = nullptr;
    fNextPlayTime = 0.0f;
...
void CArea::TAmbienceInstance::UpdateLoopSound(float fxCenter, float fyCenter,
                                               float fzCenter) {
  float fDistance = sqrtf((fx - fxCenter) * (fx - fxCenter) +
                          (fy - fyCenter) * (fy - fyCenter) +
                          (fz - fzCenter) * (fz - fzCenter));
  if (uint32_t(fDistance) < dwRange) {
    if (!playSoundInstance) {
      ...
      const char *c_szFileName = AmbienceData.AmbienceSoundVector[0].c_str();
      playSoundInstance = SoundEngine::Instance().PlayAmbienceSound3D(
          fx, fy, fz, c_szFileName, 0);
    }
...
float CArea::TAmbienceInstance::__GetVolumeFromDistance(float fDistance) {
  ...
  return 1.0f - ((fDistance - fMaxVolumeAreaRadius) /
                 (dwRange - fMaxVolumeAreaRadius));
}

void CArea::TAmbienceInstance::Render() {
  float fBoxSize = 10.0f;
  STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR, 0xff00ff00);
  RenderCube(fx - fBoxSize, fy - fBoxSize, fz - fBoxSize, fx + fBoxSize,
             fy + fBoxSize, fz + fBoxSize);
  STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR, 0xffffffff);
  RenderSphere(NULL, fx, fy, fz, float(dwRange) * fMaxVolumeAreaPercentage,
               D3DFILL_POINT);
...
bool CArea::SAmbienceInstance::Picking() {
  return CGraphicCollisionObject::IntersectSphere(D3DXVECTOR3(fx, fy, fz),
                                                  dwRange);
}
