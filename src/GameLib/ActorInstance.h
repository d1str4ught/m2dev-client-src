...
//		Note : 캐릭터의 Lighting, Local Point Light, Weapon Trace 등의
// 효과와 콤보 데이타, 사운드 데이타,
//             모션 데이타 들을 추상적, 총체적으로 관리하는 개별 매니저가 될
//             것이다.
...
public:
  ...
  public:
    ...
    virtual void OnUseSkill(const SState &c_rkState, UINT uMotSkill,
                            UINT uMotLoopCount) = 0;

    virtual void OnHit(UINT uSkill, CActorInstance &rkActorVictim,
                       BOOL isSendPacket) = 0;
...
public:
  ...
  typedef TCollisionPointInstanceList::iterator
      TCollisionPointInstanceListIterator;
...
public:
  ...
  void AttachWeapon(DWORD dwItemIndex,
                    DWORD dwParentPartIndex = CRaceData::PART_MAIN,
                    DWORD dwPartIndex = CRaceData::PART_WEAPON);
  void AttachWeapon(DWORD dwParentPartIndex, DWORD dwPartIndex,
                    CItemData *pItemData);
  ...
  DWORD AttachEffectByName(DWORD dwParentPartIndex, const char *c_pszBoneName,
                           const char *c_pszEffectFileName);
  DWORD AttachEffectByID(DWORD dwParentPartIndex, const char *c_pszBoneName,
                         DWORD dwEffectID,
                         const D3DXVECTOR3 *c_pv3Position = NULL);
  DWORD AttachSmokeEffect(DWORD eSmoke);

  /////////////////////////////////////////////////////////////////////////////////////
  // Motion Queueing System
  void SetMotionMode(
      int iMotionMode); // FIXME : 모드의 시간차 적용이 가능하게끔 한다.
  int GetMotionMode();
  void SetLoopMotion(DWORD dwMotion, float fBlendTime = 0.1f,
                     float fSpeedRatio = 1.0f);
  bool InterceptOnceMotion(DWORD dwMotion, float fBlendTime = 0.1f,
                           UINT uSkill = 0, float fSpeedRatio = 1.0f);
  bool InterceptLoopMotion(DWORD dwMotion, float fBlendTime = 0.1f);
  bool
  PushOnceMotion(DWORD dwMotion, float fBlendTime = 0.1f,
                 float fSpeedRatio =
                     1.0f); // FIXME : 모드의 시간차 적용이 가능하게끔 한다.
  bool
  PushLoopMotion(DWORD dwMotion, float fBlendTime = 0.1f,
                 float fSpeedRatio =
                     1.0f); // FIXME : 모드의 시간차 적용이 가능하게끔 한다.
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  // Collison Detection
  bool CreateCollisionInstancePiece(
  ...
  virtual bool
  TestCollisionWithDynamicSphere(const CDynamicSphereInstance &dsi);
  ...
  BOOL IsClickableDistanceDestInstance(CActorInstance &rkInstDst,
                                       float fDistance);
...
public:
  ...
  // Input
  // 하위로 옮길 가능성이 있는 코드들
  // 네트웍 연동시 전투 관련은 플레이어를 제외하곤 단순히 Showing Type이기
  // 때문에 조건 검사가 필요 없다.
  void InputNormalAttackCommand(
      float fDirRot); // Process input - Only used by player's character
  bool InputComboAttackCommand(
      float fDirRot); // Process input - Only used by player's character
  ...
  void SetBlendingPosition(const TPixelPosition &c_rPosition,
                           float fBlendingTime = 1.0f);
  ...
  void MotionEventProcess(DWORD dwcurTime, int iIndex,
...
protected:
  void __AdjustCollisionMovement(
...
protected:
  ...
  float GetLastMotionTime(
      float fBlendTime); // NOTE : 자동으로 BlendTime만큼을 앞당긴 시간을 리턴
  float GetMotionDuration(DWORD dwMotionKey);

  bool InterceptMotion(EMotionPushType iMotionType, WORD wMotion,
                       float fBlendTime = 0.1f, UINT uSkill = 0,
                       float fSpeedRatio = 1.0f);
  void PushMotion(EMotionPushType iMotionType, DWORD dwMotionKey,
                  float fBlendTime, float fSpeedRatio = 1.0f);
  void ProcessMotionEventEffectEvent(
      const CRaceMotionData::TMotionEventData *c_pData);
  void ProcessMotionEventEffectToTargetEvent(
      const CRaceMotionData::TMotionEventData *c_pData);
  void ProcessMotionEventSpecialAttacking(
      int iMotionEventIndex, const CRaceMotionData::TMotionEventData *c_pData);
  void
  ProcessMotionEventSound(const CRaceMotionData::TMotionEventData *c_pData);
...
protected:
  ...
  void __ProcessDataAttackSuccess(const NRaceData::TAttackData &c_rAttackData,
                                  CActorInstance &rVictim,
                                  const D3DXVECTOR3 &c_rv3Position,
                                  UINT uiSkill = 0, BOOL isSendPacket = TRUE);
  void __ProcessMotionEventAttackSuccess(DWORD dwMotionKey, BYTE byEventIndex,
                                         CActorInstance &rVictim);
  void __ProcessMotionAttackSuccess(DWORD dwMotionKey, CActorInstance &rVictim);

  // MR-3: Shaman on-mount hitting fix
  float __GetInvisibleTimeAdjust(const UINT uiSkill,
                                 const NRaceData::TAttackData &c_rAttackData);
...
protected:
  ...
  DWORD __SetMotion(const SSetMotionData &c_rkSetMotData,
                    DWORD dwRandMotKey = 0); // 모션 데이터 설정
  void __ClearMotion();
