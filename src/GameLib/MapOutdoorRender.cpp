...
CMapOutdoor::TTerrainNumVector
    CMapOutdoor::FSortPatchDrawStructWithTerrainNum::m_TerrainNumVector;
...
void CMapOutdoor::__RenderTerrain_RecurseRenderQuadTree(
    CTerrainQuadtreeNode *Node, bool bCullCheckNeed) {
  if (bCullCheckNeed) {
    switch (__RenderTerrain_RecurseRenderQuadTree_CheckBoundingCircle(
  ...
  if (Node->Size == 1) {
    ...
    float fDistance = fMAX(fabs(v3Center.x + m_fXforDistanceCaculation),
                           fabs(-v3Center.y + m_fYforDistanceCaculation));
...
int CMapOutdoor::__RenderTerrain_RecurseRenderQuadTree_CheckBoundingCircle(
...
void CMapOutdoor::__RenderTerrain_AppendPatch(const D3DXVECTOR3 &c_rv3Center,
                                              float fDistance, long lPatchNum) {
  assert(NULL != m_pTerrainPatchProxyList &&
         "CMapOutdoor::__RenderTerrain_AppendPatch");
...
// Splat 개수 제한
void CMapOutdoor::SetSplatLimit(int iSplatNum) { m_iSplatLimit = iSplatNum; }

std::vector<int> &CMapOutdoor::GetRenderedSplatNum(int *piPatch, int *piSplat,
                                                   float *pfSplatRatio) {
...
CArea::TCRCWithNumberVector &CMapOutdoor::GetRenderedGraphicThingInstanceNum(
    DWORD *pdwGraphicThingInstanceNum, DWORD *pdwCRCNum) {
...
void CMapOutdoor::RenderBeforeLensFlare() {
  ...
  if (!mc_pEnvironmentData) {
    TraceError(
        "CMapOutdoor::RenderBeforeLensFlare mc_pEnvironmentData is NULL");
    return;
  }

  m_LensFlare.Compute(
      mc_pEnvironmentData->DirLights[ENV_DIRLIGHT_BACKGROUND].Direction);
}
...
void CMapOutdoor::RenderScreenFiltering() { m_ScreenFilter.Render(); }
...
void CMapOutdoor::SetInverseViewAndDynamicShaodwMatrices() {
  ...
  D3DXVECTOR3 v3LightEye(v3Target.x - 1.732f * 1250.0f, v3Target.y - 1250.0f,
  ...
  m_matDynamicShadow =
      m_matViewInverse * m_matLightView * m_matDynamicShadowScale;
}
...
struct FPCBlockerHide {
  void operator()(CGraphicObjectInstance *pInstance) { pInstance->Hide(); }
};
...
struct FRenderPCBlocker {
  void operator()(CGraphicObjectInstance *pInstance) {
    ...
    CGraphicThingInstance *pThingInstance =
        dynamic_cast<CGraphicThingInstance *>(pInstance);
...
struct CMapOutdoor_LessThingInstancePtrRenderOrder {
  bool operator()(CGraphicThingInstance *pkLeft,
                  CGraphicThingInstance *pkRight) {
...
void CMapOutdoor::RenderArea(bool bRenderAmbience) {
  ...
  // PCBlocker
  std::for_each(m_PCBlockerVector.begin(), m_PCBlockerVector.end(),
                FPCBlockerHide());
  ...
  if (m_bDrawShadow && m_bDrawChrShadow) {
    ...
    std::for_each(m_ShadowReceiverVector.begin(), m_ShadowReceiverVector.end(),
                  FAreaRenderShadow());
    ...
    if (mc_pEnvironmentData != NULL)
      STATEMANAGER.SetRenderState(D3DRS_FOGCOLOR,
                                  mc_pEnvironmentData->FogColor);
  }
  ...
  if (m_isDisableSortRendering) {
    for (int i = 0; i < AROUND_AREA_NUM; ++i) {
      ...
      if (GetAreaPointer(i, &pArea)) {
        ...
        m_dwRenderedGraphicThingInstanceNum +=
            pArea->DEBUG_GetRenderedGrapphicThingInstanceNum();

        CArea::TCRCWithNumberVector &rCRCWithNumberVector =
            pArea->DEBUG_GetRenderedCRCWithNumVector();

        CArea::TCRCWithNumberVector::iterator aIterator =
            rCRCWithNumberVector.begin();
        while (aIterator != rCRCWithNumberVector.end()) {
          ...
          CArea::TCRCWithNumberVector::iterator aCRCWithNumberVectorIterator =
              std::find_if(m_dwRenderedCRCWithNumberVector.begin(),
                           m_dwRenderedCRCWithNumberVector.end(),
                           CArea::FFindIfCRC(dwCRC));

          if (m_dwRenderedCRCWithNumberVector.end() ==
            ...
            CArea::TCRCWithNumber &rCRCWithNumber =
                *aCRCWithNumberVectorIterator;
    ...
    std::sort(m_dwRenderedCRCWithNumberVector.begin(),
              m_dwRenderedCRCWithNumberVector.end(), CArea::CRCNumComp());
  } else {
    static std::vector<CGraphicThingInstance *> s_kVct_pkOpaqueThingInstSort;
    s_kVct_pkOpaqueThingInstSort.clear();
    s_kVct_pkOpaqueThingInstSort.reserve(
        512); // Pre-allocate to avoid reallocations
    ...
    std::sort(s_kVct_pkOpaqueThingInstSort.begin(),
  ...
  if (m_bDrawShadow && m_bDrawChrShadow) {
    std::for_each(m_ShadowReceiverVector.begin(), m_ShadowReceiverVector.end(),
                  std::mem_fn(&CGraphicObjectInstance::Show));
  }
}
...
void CMapOutdoor::RenderBlendArea() {
  ...
  s_kVct_pkBlendThingInstSort.reserve(
      256); // Pre-allocate to avoid reallocations
  ...
  if (s_kVct_pkBlendThingInstSort.size() != 0) {
    ...
    // STATEMANAGER.SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_DISABLE);
    // STATEMANAGER.SaveTextureStageState(1, D3DTSS_TEXCOORDINDEX,
    // D3DTSS_TCI_CAMERASPACEPOSITION); STATEMANAGER.SaveTextureStageState(1,
    ...
    // STATEMANAGER.SetTextureStageState(1, D3DTSS_COLOROP,   D3DTOP_MODULATE);
    // STATEMANAGER.SetTextureStageState(1, D3DTSS_ALPHAOP,   D3DTOP_DISABLE);
    // STATEMANAGER.SaveTextureStageState(1, D3DTSS_ADDRESSU,
    // D3DTADDRESS_BORDER); STATEMANAGER.SaveTextureStageState(1,
    // D3DTSS_ADDRESSV, D3DTADDRESS_BORDER);
    // STATEMANAGER.SaveTextureStageState(1, D3DTSS_BORDERCOLOR, 0xFFFFFFFF);

    ////std::for_each(m_ShadowReceiverVector.begin(),
    /// m_ShadowReceiverVector.end(), FAreaRenderShadow());
    ...
    std::sort(s_kVct_pkBlendThingInstSort.begin(),
              s_kVct_pkBlendThingInstSort.end(),
              CMapOutdoor_LessThingInstancePtrRenderOrder());
    ...
    std::for_each(s_kVct_pkBlendThingInstSort.begin(),
                  s_kVct_pkBlendThingInstSort.end(),
...
void CMapOutdoor::RenderPCBlocker() {
  ...
  if (m_PCBlockerVector.size() != 0) {
    ...
    STATEMANAGER.SetTexture(
        1, m_BuildingTransparentImageInstance.GetTexturePointer()
               ->GetD3DTexture());
...
void CMapOutdoor::SelectIndexBuffer(BYTE byLODLevel, WORD *pwPrimitiveCount,
...
void CMapOutdoor::SetPatchDrawVector() {
  assert(NULL != m_pTerrainPatchProxyList &&
         "CMapOutdoor::__SetPatchDrawVector");
  ...
  while (aDistancePatchVectorIterator != m_PatchVector.end()) {
    ...
    CTerrainPatchProxy *pTerrainPatchProxy =
        &m_pTerrainPatchProxyList[adistancePatchPair.second];
  ...
  std::stable_sort(m_PatchDrawStructVector.begin(),
                   m_PatchDrawStructVector.end(),
...
struct FPatchNumMatch {
  ...
  FPatchNumMatch(long lPatchNum) { m_lPatchNumToCheck = lPatchNum; }
  bool operator()(std::pair<long, BYTE> aPair) {
    return m_lPatchNumToCheck == aPair.first;
  }
};

void CMapOutdoor::NEW_DrawWireFrame(CTerrainPatchProxy *pTerrainPatchProxy,
...
void CMapOutdoor::DrawWireFrame(long patchnum, WORD wPrimitiveCount,
                                D3DPRIMITIVETYPE ePrimitiveType) {
...
void CMapOutdoor::RenderMarkedArea() {
  ...
  D3DXMatrixScaling(&matTexTransform, m_fTerrainTexCoordBase * 32.0f,
                    -m_fTerrainTexCoordBase * 32.0f, 0.0f);
  ...
  STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR,
                              D3DXCOLOR(1.0f, 1.0f, 1.0f, fAlpha));
  STATEMANAGER.SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
  ...
  STATEMANAGER.SetTexture(
      0, m_attrImageInstance.GetTexturePointer()->GetD3DTexture());
...
void CMapOutdoor::RecurseRenderAttr(CTerrainQuadtreeNode *Node,
                                    bool bCullEnable) {
  if (bCullEnable) {
    if (__RenderTerrain_RecurseRenderQuadTree_CheckBoundingCircle(
            Node->center, Node->radius) == VIEW_NONE)
      return;
  }
...
void CMapOutdoor::DrawPatchAttr(long patchnum) {
  ...
  m_matWorldForCommonUse._41 =
      -(float)(wCoordX * CTerrainImpl::XSIZE * CTerrainImpl::CELLSCALE);
  m_matWorldForCommonUse._42 =
      (float)(wCoordY * CTerrainImpl::YSIZE * CTerrainImpl::CELLSCALE);
  ...
  STATEMANAGER.DrawIndexedPrimitive(D3DPT_TRIANGLESTRIP, 0,
                                    m_iPatchTerrainVertexCount, 0,
                                    m_wNumIndices[0] - 2);
}
