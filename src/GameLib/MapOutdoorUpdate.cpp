...
// 2004.08.17.myevan.std::vector 를 사용할 경우 메모리 접근에 오래걸려
// 스택쪽으로 계산하도록 수정
...
bool CMapOutdoor::Update(float fX, float fY, float fZ) {
  ...
  short sCoordX =
      MINMAX(0, ix / CTerrainImpl::TERRAIN_XSIZE, m_sTerrainCountX - 1);
  short sCoordY =
      MINMAX(0, iy / CTerrainImpl::TERRAIN_YSIZE, m_sTerrainCountY - 1);
#ifdef __PERFORMANCE_CHECKER__
  DWORD t2 = ELTimer_GetMSec();
#endif

  bool bNeedInit = (m_PrevCoordinate.m_sTerrainCoordX == -1 ||
                    m_PrevCoordinate.m_sTerrainCoordY == -1);

  if (bNeedInit ||
      (m_CurCoordinate.m_sTerrainCoordX / LOAD_SIZE_WIDTH) !=
          (sCoordX / LOAD_SIZE_WIDTH) ||
      (m_CurCoordinate.m_sTerrainCoordY / LOAD_SIZE_WIDTH) !=
          (sCoordY / LOAD_SIZE_WIDTH)) {
    if (bNeedInit) {
    ...
    WORD wCellCoordX =
        (ix % CTerrainImpl::TERRAIN_XSIZE) / CTerrainImpl::CELLSCALE;
    WORD wCellCoordY =
        (iy % CTerrainImpl::TERRAIN_YSIZE) / CTerrainImpl::CELLSCALE;

    short sReferenceCoordMinX, sReferenceCoordMaxX, sReferenceCoordMinY,
        sReferenceCoordMaxY;
    sReferenceCoordMinX =
        std::max(m_CurCoordinate.m_sTerrainCoordX - LOAD_SIZE_WIDTH, 0);
    sReferenceCoordMaxX =
        std::min(m_CurCoordinate.m_sTerrainCoordX + LOAD_SIZE_WIDTH,
                 m_sTerrainCountX - 1);
    sReferenceCoordMinY =
        std::max(m_CurCoordinate.m_sTerrainCoordY - LOAD_SIZE_WIDTH, 0);
    sReferenceCoordMaxY =
        std::min(m_CurCoordinate.m_sTerrainCoordY + LOAD_SIZE_WIDTH,
                 m_sTerrainCountY - 1);
...
#endif
  CSpeedTreeForestDirectX8::Instance().UpdateSystem(
...
void CMapOutdoor::UpdateSky() { m_SkyBox.Update(); }
...
struct FGetShadowReceiverFromCollisionData {
  ...
  void operator()(CGraphicObjectInstance *pInstance) {
    ...
    if (TREE_OBJECT == pInstance->GetType() ||
        ACTOR_OBJECT == pInstance->GetType() ||
...
struct FPCBlockerDistanceSort {
  ...
  bool operator()(CGraphicObjectInstance *plhs,
                  CGraphicObjectInstance *prhs) const {
...
void CMapOutdoor::__Game_UpdateArea(D3DXVECTOR3 &v3Player) {
  ...
  {
          printf("%.2f %.2f %.2f -> target(%.2f %.2f %.2f) player(%.2f %.2f
  %.2f)\n", v3Eye.x, v3Eye.y, v3Eye.z, v3Target.x, v3Target.y, v3Target.z,
...
struct FGetShadowReceiverFromHeightData {
  ...
  FGetShadowReceiverFromHeightData(float fFromX, float fFromY, float fToX,
                                   float fToY)
      : m_fFromX(fFromX), m_fFromY(fFromY), m_fToX(fToX), m_fToY(fToY),
        m_bReceiverFound(false) {
    m_dwCheckCount = 0;
  ...
  UINT GetCollectCount() { return m_dwCollectCount; }
...
void CMapOutdoor::__CollectShadowReceiver(D3DXVECTOR3 &v3Target,
                                          D3DXVECTOR3 &v3Light) {
  ...
  FGetShadowReceiverFromHeightData kGetShadowReceiverFromHeightData(
      v3Target.x, v3Target.y, s.v3Position.x, s.v3Position.y);
  ...
  if (kGetShadowReceiverFromHeightData.m_bReceiverFound) {
    for (UINT i = 0; i < kGetShadowReceiverFromHeightData.GetCollectCount();
         ++i) {
      CGraphicObjectInstance *pObjInstEach =
          kGetShadowReceiverFromHeightData.GetCollectItem(i);
...
struct PCBlocker_SInstanceList {
  ...
  Iterator Begin() { return m_apkPCBlocker; }
  Iterator End() { return m_apkPCBlocker + m_dwBlockerCount; }

  DWORD Size() { return m_dwBlockerCount; }
...
void CMapOutdoor::__CollectCollisionPCBlocker(D3DXVECTOR3 &v3Eye,
  ...
  DWORD t5 = timeGetTime();
#endif
  std::sort(m_PCBlockerVector.begin(), m_PCBlockerVector.end(),
            FPCBlockerDistanceSort(v3Eye));
  ...
  if (t3 - t1 > 5) {
    ...
    fprintf(fp, "PBC.Count (Collect %d, Over %d, Check %d)\n",
            ...
            kPCBlockerList.m_dwBlockerOverCount, kPCBlockerList.m_dwInstCount);
    fflush(fp);
  }
#endif
}

void CMapOutdoor::__CollectCollisionShadowReceiver(D3DXVECTOR3 &v3Target,
                                                   D3DXVECTOR3 &v3Light) {
  CDynamicSphereInstance s;
  s.fRadius = 50.0f;
  s.v3LastPosition = v3Target + v3Light;
  ...
  FGetShadowReceiverFromCollisionData kGetShadowReceiverFromCollisionData(
      &s, &kVct_pkShadowReceiver);
  rkCullingMgr.ForInRange(aVector3d, 100.0f,
                          &kGetShadowReceiverFromCollisionData);
  if (!kGetShadowReceiverFromCollisionData.m_bCollide)
    return;

  std::vector<CGraphicObjectInstance *>::iterator i;
  for (i = kVct_pkShadowReceiver.begin(); i != kVct_pkShadowReceiver.end();
       ++i) {
    CGraphicObjectInstance *pObjInstEach = *i;
    if (!__IsInPCBlockerList(pObjInstEach))
      if (!__IsInShadowReceiverList(pObjInstEach))
...
bool CMapOutdoor::__IsInShadowReceiverList(
    CGraphicObjectInstance *pkObjInstTest) {
  if (m_ShadowReceiverVector.end() == std::find(m_ShadowReceiverVector.begin(),
...
bool CMapOutdoor::__IsInPCBlockerList(CGraphicObjectInstance *pkObjInstTest) {
  if (m_PCBlockerVector.end() == std::find(m_PCBlockerVector.begin(),
                                           m_PCBlockerVector.end(),
                                           pkObjInstTest))
    return false;
...
void CMapOutdoor::UpdateTerrain(float fX, float fY) {
  ...
  if ((m_lCenterX != m_lOldReadX) || (m_lCenterY != m_lOldReadY)) {
    ...
    // Tracef("사용하는 Area, Terrain : (%d, %d), 지울 Area, Terrain : (%d,
    // %d)\n", 	m_AreaVector.size(), m_TerrainVector.size(),
    // m_AreaDeleteVector.size(), m_TerrainDeleteVector.size());
  }
}

void CMapOutdoor::FPushTerrainToDeleteVector::operator()(CTerrain *pTerrain) {
  TTerrainPtrVectorIterator aIterator = std::find(
      m_ReturnTerrainVector.begin(), m_ReturnTerrainVector.end(), pTerrain);
  ...
  aIterator = std::find(m_ReturnTerrainVector.begin(),
                        m_ReturnTerrainVector.end(), pTerrain);
  if (aIterator != m_ReturnTerrainVector.end())
    return;
...
void CMapOutdoor::FPushAreaToDeleteVector::operator()(CArea *pArea) {
  TAreaPtrVectorIterator aIterator =
      std::find(m_ReturnAreaVector.begin(), m_ReturnAreaVector.end(), pArea);
...
void CMapOutdoor::__ClearGarvage() {
  std::for_each(m_TerrainDeleteVector.begin(), m_TerrainDeleteVector.end(),
                CTerrain::Delete);
  m_TerrainDeleteVector.clear();

  std::for_each(m_AreaDeleteVector.begin(), m_AreaDeleteVector.end(),
                CArea::Delete);
...
void CMapOutdoor::__UpdateGarvage() {
  ...
  if (!m_TerrainDeleteVector.empty()) {
    if (ELTimer_GetMSec() - dwEraseTime <= dwTerrainEraseInterval)
      return;
    TTerrainPtrVectorIterator aTerrainPtrDeleteItertor =
        m_TerrainDeleteVector.begin();
    CTerrain *pTerrain = *aTerrainPtrDeleteItertor;
    CTerrain::Delete(pTerrain);

    aTerrainPtrDeleteItertor =
        m_TerrainDeleteVector.erase(aTerrainPtrDeleteItertor);
...
void CMapOutdoor::UpdateAreaList(long lCenterX, long lCenterY) {
  if (m_TerrainVector.size() <= AROUND_AREA_NUM &&
      m_AreaVector.size() <= AROUND_AREA_NUM)
    return;
  ...
  FPushTerrainToDeleteVector rPushTerrainToDeleteVector = std::for_each(
      m_TerrainVector.begin(), m_TerrainVector.end(),
      FPushTerrainToDeleteVector(eDeleteLRDir, eDeleteTBDir, m_CurCoordinate));
  FPushAreaToDeleteVector rPushAreaToDeleteVector = std::for_each(
      m_AreaVector.begin(), m_AreaVector.end(),
  ...
  if (!rPushTerrainToDeleteVector.m_ReturnTerrainVector.empty()) {
    m_TerrainDeleteVector.resize(
        rPushTerrainToDeleteVector.m_ReturnTerrainVector.size());
    std::copy(rPushTerrainToDeleteVector.m_ReturnTerrainVector.begin(),
              rPushTerrainToDeleteVector.m_ReturnTerrainVector.end(),
              m_TerrainDeleteVector.begin());

    for (DWORD dwIndex = 0;
         dwIndex < rPushTerrainToDeleteVector.m_ReturnTerrainVector.size();
         ++dwIndex) {
      bool isDel = false;
      ...
      while (aTerrainPtrItertor != m_TerrainVector.end()) {
        ...
        if (pTerrain ==
            rPushTerrainToDeleteVector.m_ReturnTerrainVector[dwIndex]) {
  ...
  if (!rPushAreaToDeleteVector.m_ReturnAreaVector.empty()) {
    m_AreaDeleteVector.resize(
        rPushAreaToDeleteVector.m_ReturnAreaVector.size());
    std::copy(rPushAreaToDeleteVector.m_ReturnAreaVector.begin(),
              rPushAreaToDeleteVector.m_ReturnAreaVector.end(),
              m_AreaDeleteVector.begin());

    for (DWORD dwIndex = 0;
         dwIndex < rPushAreaToDeleteVector.m_ReturnAreaVector.size();
         ++dwIndex) {
...
void CMapOutdoor::ConvertTerrainToTnL(long lx, long ly) {
  assert(NULL != m_pTerrainPatchProxyList &&
         "CMapOutdoor::ConvertTerrainToTnL");
...
void CMapOutdoor::AssignPatch(long lPatchNum, long x0, long y0, long x1,
                              long y1) {
  ...
  CTerrainPatch *pTerrainPatch =
      pTerrain->GetTerrainPatchPtr(byPatchNumX, byPatchNumY);
  if (!pTerrainPatch)
    return;

  pTerrainPatchProxy->SetPatchNum(byPatchNumY * CTerrainImpl::PATCH_XCOUNT +
                                  byPatchNumX);
...
void CMapOutdoor::UpdateQuadTreeHeights(CTerrainQuadtreeNode *Node) {
  // Inserted by levites
  assert(NULL != m_pTerrainPatchProxyList &&
         "CMapOutdoor::UpdateQuadTreeHeights");
  ...
  Node->radius =
      sqrtf((maxx - minx) * (maxx - minx) + (maxy - miny) * (maxy - miny) +
            (maxz - minz) * (maxz - minz)) /
      2.0f;
