...
struct FGetPickingPoint {
  ...
  FGetPickingPoint(D3DXVECTOR3 &v3Start, D3DXVECTOR3 &v3Dir)
...
CMapOutdoor::CMapOutdoor() {
  CGraphicImage *pAttrImage =
      (CGraphicImage *)CResourceManager::Instance().GetResourcePointer(
          "d:/ymir work/special/white.dds");
  CGraphicImage *pBuildTransparentImage =
      (CGraphicImage *)CResourceManager::Instance().GetResourcePointer(
          "d:/ymir Work/special/PCBlockerAlpha.dds");
  ...
  // DX11 terrain shader — initialize now that device is ready
  m_pDX11TerrainShader = nullptr;
  if (ms_pD3D11Device && ms_pD3D11Context) {
    m_pDX11TerrainShader = new CDX11TerrainShader();
    if (!m_pDX11TerrainShader->Initialize(ms_pD3D11Device, ms_pD3D11Context)) {
      TraceError("CMapOutdoor: DX11TerrainShader initialization failed");
      delete m_pDX11TerrainShader;
      m_pDX11TerrainShader = nullptr;
    }
  }
}
...
CMapOutdoor::~CMapOutdoor() {
  ...
  if (m_pDX11TerrainShader) {
    m_pDX11TerrainShader->Shutdown();
    delete m_pDX11TerrainShader;
    m_pDX11TerrainShader = nullptr;
  }
...
void CMapOutdoor::OnBeginEnvironment() {
  ...
  const D3DLIGHT9 &c_rkLight =
      mc_pEnvironmentData->DirLights[ENV_DIRLIGHT_CHARACTER];
  rkForest.SetLight((const float *)&c_rkLight.Direction,
...
void CMapOutdoor::SetEnvironmentScreenFilter() {
  ...
  m_ScreenFilter.SetBlendType(mc_pEnvironmentData->byFilteringAlphaSrc,
                              mc_pEnvironmentData->byFilteringAlphaDest);
...
void CMapOutdoor::SetEnvironmentSkyBox() {
  ...
  m_SkyBox.SetGradientLevel(mc_pEnvironmentData->bySkyBoxGradientLevelUpper,
                            mc_pEnvironmentData->bySkyBoxGradientLevelLower);
  m_SkyBox.SetRenderMode((mc_pEnvironmentData->bSkyBoxTextureRenderMode == TRUE)
                             ? CSkyObject::SKY_RENDER_MODE_TEXTURE
                             : CSkyObject::SKY_RENDER_MODE_DIFFUSE);

  for (int i = 0; i < 6; ++i) {
    if (!mc_pEnvironmentData->strSkyBoxFaceFileName[i].empty())
      m_SkyBox.SetFaceTexture(
          mc_pEnvironmentData->strSkyBoxFaceFileName[i].c_str(), i);
  }

  if (!mc_pEnvironmentData->strCloudTextureFileName.empty())
    m_SkyBox.SetCloudTexture(
        mc_pEnvironmentData->strCloudTextureFileName.c_str());
  ...
  // Temporary
  m_SkyBox.SetCloudColor(mc_pEnvironmentData->CloudGradientColor,
                         mc_pEnvironmentData->CloudGradientColor, 1);

  if (!mc_pEnvironmentData->SkyBoxGradientColorVector.empty())
    m_SkyBox.SetSkyColor(mc_pEnvironmentData->SkyBoxGradientColorVector,
                         mc_pEnvironmentData->SkyBoxGradientColorVector, 1);
  // Temporary
...
void CMapOutdoor::SetEnvironmentLensFlare() {
  ...
  m_LensFlare.CharacterizeFlare(
      mc_pEnvironmentData->bLensFlareEnable == 1 ? true : false,
  ...
  if (!mc_pEnvironmentData->strMainFlareTextureFileName.empty())
    m_LensFlare.SetMainFlare(
        mc_pEnvironmentData->strMainFlareTextureFileName.c_str(),
...
bool CMapOutdoor::IsWireframe() { return m_bDrawWireFrame; }
...
void CMapOutdoor::CreateTerrainPatchProxyList() {
  ...
  m_pTerrainPatchProxyList =
      new CTerrainPatchProxy[m_wPatchCount * m_wPatchCount];

  m_iPatchTerrainVertexCount =
      (TERRAIN_PATCHSIZE + 1) * (TERRAIN_PATCHSIZE + 1);
...
bool CMapOutdoor::__PickTerrainHeight(float &fPos, const D3DXVECTOR3 &v3Start,
                                      const D3DXVECTOR3 &v3End, float fStep,
                                      float fRayRange, float fLimitRange,
                                      D3DXVECTOR3 *pv3Pick) {
  CTerrain *pTerrain;
...
bool CMapOutdoor::GetPickingPointWithRay(const CRay &rRay,
                                         D3DXVECTOR3 *v3IntersectPt) {
  ...
  bTerrainPick = true;
  if (!__PickTerrainHeight(fPos, v3Start, v3End, 5.0f, fRayRange, 5000.0f,
                           &v3TerrainPick))
    if (!__PickTerrainHeight(fPos, v3Start, v3End, 10.0f, fRayRange, 10000.0f,
                             &v3TerrainPick))
      if (!__PickTerrainHeight(fPos, v3Start, v3End, 100.0f, fRayRange,
                               100000.0f, &v3TerrainPick))
        bTerrainPick = false;
...
bool CMapOutdoor::GetPickingPointWithRayOnlyTerrain(
    const CRay &rRay, D3DXVECTOR3 *v3IntersectPt) {
  bool bTerrainPick = false;
...
void CMapOutdoor::GetHeightMap(const BYTE &c_rucTerrainNum,
                               WORD **pwHeightMap) {
  if (c_rucTerrainNum < 0 || c_rucTerrainNum > AROUND_AREA_NUM - 1 ||
      !m_pTerrain[c_rucTerrainNum]) {
    *pwHeightMap = NULL;
    return;
  }
...
void CMapOutdoor::GetNormalMap(const BYTE &c_rucTerrainNum,
                               char **pucNormalMap) {
  if (c_rucTerrainNum < 0 || c_rucTerrainNum > AROUND_AREA_NUM - 1 ||
      !m_pTerrain[c_rucTerrainNum]) {
    *pucNormalMap = NULL;
...
void CMapOutdoor::GetWaterHeight(BYTE byTerrainNum, BYTE byWaterNum,
                                 long *plWaterHeight) {
  if (byTerrainNum < 0 || byTerrainNum > AROUND_AREA_NUM - 1 ||
      !m_pTerrain[byTerrainNum]) {
    *plWaterHeight = -1;
    return;
  }
...
bool CMapOutdoor::GetWaterHeight(int iX, int iY, long *plWaterHeight) {
  ...
  wLocalX = (iX - wTerrainCoordX * CTerrainImpl::TERRAIN_XSIZE) /
            (CTerrainImpl::WATERMAP_XSIZE);
  wLocalY = (iY - wTerrainCoordY * CTerrainImpl::TERRAIN_YSIZE) /
            (CTerrainImpl::WATERMAP_YSIZE);
...
bool CMapOutdoor::GetTerrainNumFromCoord(WORD wCoordX, WORD wCoordY,
                                         BYTE *pbyTerrainNum) {
  *pbyTerrainNum =
      (wCoordY - m_CurCoordinate.m_sTerrainCoordY + LOAD_SIZE_WIDTH) * 3 +
...
void CMapOutdoor::BuildViewFrustum(D3DXMATRIX &mat) {
  // m_plane[0] = D3DXPLANE(mat._14 + mat._13, mat._24 + mat._23, mat._34 +
  // mat._33, mat._44 + mat._43);
  m_plane[0] = D3DXPLANE(mat._13, mat._23, mat._33, mat._43); // Near
  m_plane[1] = D3DXPLANE(mat._14 - mat._13, mat._24 - mat._23,
                         mat._34 - mat._33, mat._44 - mat._43); // Far
  m_plane[2] = D3DXPLANE(mat._14 + mat._11, mat._24 + mat._21,
                         mat._34 + mat._31, mat._44 + mat._41); // Left
  m_plane[3] = D3DXPLANE(mat._14 - mat._11, mat._24 - mat._21,
                         mat._34 - mat._31, mat._44 - mat._41); // Right
  m_plane[4] = D3DXPLANE(mat._14 + mat._12, mat._24 + mat._22,
                         mat._34 + mat._32, mat._44 + mat._42); // Bottom
  m_plane[5] = D3DXPLANE(mat._14 - mat._12, mat._24 - mat._22,
                         mat._34 - mat._32, mat._44 - mat._42); // Top
...
void CMapOutdoor::__HeightCache_Update() { m_kHeightCache.m_isUpdated = true; }
...
float CMapOutdoor::GetCacheHeight(float fx, float fy) {
  ...
  if (m_kHeightCache.m_isUpdated && nx < 16 * 30000 && ny < 16 * 30000) {
    ...
    pkVct_kItem =
        &m_kHeightCache.m_akVct_kItem[dwKey % SHeightCache::HASH_SIZE];
...
bool CMapOutdoor::GetBrushColor(float fX, float fY, float *pLowColor,
                                float *pHighColor) {
  bool bSuccess = false;
...
BOOL CMapOutdoor::GetTerrainPointer(const BYTE c_byTerrainNum,
                                    CTerrain **ppTerrain) {
  ...
  if (fYRef < -fTerrainSize)
    ...
    if (fXRef < -fTerrainSize)
      ...
      if (GetTerrainPointer(0, &pTerrain))
        return pTerrain->GetShadowMapColor(fXRef + fTerrainSize,
                                           fYRef + fTerrainSize);
      else
        return 0xFFFFFFFF;
...
bool CMapOutdoor::GetAttr(int iX, int iY, BYTE *pbyAttr) {
  ...
  WORD wLocalX, wLocalY;
  wLocalX = (WORD)(iX - wTerrainCoordX * CTerrainImpl::TERRAIN_XSIZE) /
            (CTerrainImpl::HALF_CELLSCALE);
  wLocalY = (WORD)(iY - wTerrainCoordY * CTerrainImpl::TERRAIN_YSIZE) /
            (CTerrainImpl::HALF_CELLSCALE);
...
// MonsterAreaInfo
CMonsterAreaInfo *CMapOutdoor::AddMonsterAreaInfo(long lOriginX, long lOriginY,
...
void CMapOutdoor::RemoveAllMonsterAreaInfo() {
  ...
  while (m_MonsterAreaInfoPtrVectorIterator !=
         m_MonsterAreaInfoPtrVector.end()) {
...
bool CMapOutdoor::GetMonsterAreaInfoFromVectorIndex(
    DWORD dwMonsterAreaInfoVectorIndex, CMonsterAreaInfo **ppMonsterAreaInfo) {
...
//////////////////////////////////////////////////////////////////////////
CMonsterAreaInfo *CMapOutdoor::AddNewMonsterAreaInfo(
    long lOriginX, long lOriginY, long lSizeX, long lSizeY,
    CMonsterAreaInfo::EMonsterAreaInfoType eMonsterAreaInfoType, DWORD dwVID,
    DWORD dwCount, CMonsterAreaInfo::EMonsterDir eMonsterDir) {
  CMonsterAreaInfo *pMonsterAreaInfo = m_kPool_kMonsterAreaInfo.Alloc();
  pMonsterAreaInfo->Clear();
  pMonsterAreaInfo->SetOrigin(lOriginX, lOriginY);
  pMonsterAreaInfo->SetSize(lSizeX, lSizeY);
...
void CMapOutdoor::SetEnvironmentDataName(
    const std::string &strEnvironmentDataName) {
...
void CMapOutdoor::__XMasTree_Create(float x, float y, float z,
                                    const char *c_szTreeName,
                                    const char *c_szEffName) {
  ...
  m_kXMas.m_iEffectID = rkEffMgr.CreateEffect(c_szEffName, D3DXVECTOR3(x, y, z),
                                              D3DXVECTOR3(0.0f, 0.0f, 0.0f));
}

void CMapOutdoor::XMasTree_Set(float x, float y, float z,
                               const char *c_szTreeName,
                               const char *c_szEffName) {
  XMasTree_Destroy();
...
void CMapOutdoor::SpecialEffect_Create(DWORD dwID, float x, float y, float z,
                                       const char *c_szEffName) {
  CEffectManager &rkEffMgr = CEffectManager::Instance();
  ...
  rkEffMgr.RegisterEffect(c_szEffName);
  DWORD dwEffectID = rkEffMgr.CreateEffect(c_szEffName, D3DXVECTOR3(x, y, z),
                                           D3DXVECTOR3(0.0f, 0.0f, 0.0f));
...
void CMapOutdoor::ClearGuildArea() { m_rkList_kGuildArea.clear(); }
...
void CMapOutdoor::VisibleMarkedArea() {
  ...
  for (; itorRect != m_rkList_kGuildArea.end(); ++itorRect) {
    ...
    ConvertToMapCoords(float(rkRect.left), float(rkRect.top), &ix1Cell,
                       &iy1Cell, &byx1SubCell, &byy1SubCell, &wx1TerrainNum,
                       &wy1TerrainNum);
    ConvertToMapCoords(float(rkRect.right), float(rkRect.bottom), &ix2Cell,
                       &iy2Cell, &byx2SubCell, &byy2SubCell, &wx2TerrainNum,
                       &wy2TerrainNum);
    ...
    for (int ixCell = ix1Cell; ixCell <= ix2Cell; ++ixCell)
      for (int iyCell = iy1Cell; iyCell <= iy2Cell; ++iyCell) {
        ...
        if (kMap_pbyMarkBuf.end() == kMap_pbyMarkBuf.find(iTerrainNum)) {
          BYTE *pbyBuf =
              new BYTE[CTerrain::ATTRMAP_XSIZE * CTerrain::ATTRMAP_YSIZE];
...
void CMapOutdoor::ConvertToMapCoords(float fx, float fy, int *iCellX,
                                     int *iCellY, BYTE *pucSubCellX,
                                     BYTE *pucSubCellY, WORD *pwTerrainNumX,
                                     WORD *pwTerrainNumY) {
  if (fy < 0)
    fy = -fy;
