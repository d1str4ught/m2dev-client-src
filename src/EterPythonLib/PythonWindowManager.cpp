...
CWindowManager::CWindowManager()
    : m_pActiveWindow(NULL), m_pPointWindow(NULL), m_pLeftCaptureWindow(NULL),
      m_pMiddleCaptureWindow(NULL), m_pRightCaptureWindow(NULL),
      m_pLockWindow(NULL), m_bAttachingFlag(FALSE), m_dwAttachingType(0),
      m_dwAttachingIndex(0), m_dwAttachingSlotNumber(0), m_poMouseHandler(NULL),
      m_iHres(0), m_iVres(0), m_bOnceIgnoreMouseLeftButtonUpEventFlag(FALSE) {
  ...
  for (DWORD layer = 0; layer < sizeof(layerTbl) / sizeof(layerTbl[0]);
       layer++) {
    CWindow *pLayer = new CLayer(NULL);
    pLayer->SetName(layerTbl[layer]);
    pLayer->Show();
    m_LayerWindowMap.insert(
        TLayerContainer::value_type(layerTbl[layer], pLayer));
...
CWindow *CWindowManager::RegisterTypeWindow(PyObject *po, DWORD dwWndType,
                                            const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
CWindow *CWindowManager::RegisterSlotWindow(PyObject *po,
                                            const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
CWindow *CWindowManager::RegisterGridSlotWindow(PyObject *po,
                                                const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
CWindow *CWindowManager::RegisterExpandedImageBox(PyObject *po,
                                                  const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
CWindow *CWindowManager::RegisterAniImageBox(PyObject *po,
                                             const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
CWindow *CWindowManager::RegisterRadioButton(PyObject *po,
                                             const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
CWindow *CWindowManager::RegisterToggleButton(PyObject *po,
                                              const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
CWindow *CWindowManager::RegisterDragButton(PyObject *po,
                                            const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
CWindow *CWindowManager::RegisterNumberLine(PyObject *po,
                                            const char *c_szLayer) {
  assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));
...
BOOL CWindowManager::IsAttaching() { return m_bAttachingFlag; }

DWORD CWindowManager::GetAttachingType() { return m_dwAttachingType; }

DWORD CWindowManager::GetAttachingIndex() { return m_dwAttachingIndex; }
...
void CWindowManager::AttachIcon(DWORD dwType, DWORD dwIndex, DWORD dwSlotNumber,
                                BYTE byWidth, BYTE byHeight) {
...
void CWindowManager::ActivateWindow(CWindow *pWin) {
  ...
  if (m_pActiveWindow) {
    ...
    //        일단은 중복 누적이 안되며 포커스 되는 갯수 자체가 5개 미만이니
    //        굳이 필요하지는 않을 듯.. - [levites]
...
void CWindowManager::SetTop(CWindow *pWin) {
  ...
  // NOTE : 인벤토리에서 아이템을 드래그 해서 밖에다 놓을때 캡춰가 남아서 창의
  // 버튼을 두번 눌러야 하는 버그를 위해 추가
  //		ResetCapture();
}
...
CWindow *CWindowManager::GetActivateWindow() { return m_pActiveWindow; }
...
void CWindowManager::ClearStoredSlotCoolTimeInAllSlotWindows(
    DWORD dwKey, DWORD dwSlotIndex) {
  // recursively walk the window tree starting from layers and clear stored
  // cooldown entries
...
float CWindowManager::GetAspect() { return (m_iHres) / float(m_iVres); }

void CWindowManager::SetScreenSize(long lWidth, long lHeight) {
  m_lWidth = lWidth;
  m_lHeight = lHeight;

  for (TLayerContainer::iterator itor = m_LayerWindowMap.begin();
       itor != m_LayerWindowMap.end(); ++itor) {
...
void CWindowManager::Render() { m_pRootWindow->Render(); }
...
CWindow *CWindowManager::__PickWindow(long x, long y) {
  ...
  for (TWindowContainer::iterator itor = m_PickAlwaysWindowList.begin();
       itor != m_PickAlwaysWindowList.end(); ++itor) {
  ...
  for (TWindowContainer::reverse_iterator ritor = m_LayerWindowList.rbegin();
       ritor != m_LayerWindowList.rend(); ++ritor) {
...
void CWindowManager::RunKeyDown(int vkey) {
  ...
  if (pKeyCaptureWindow)
    if (m_ReserveDeleteWindowList.end() ==
        std::find(m_ReserveDeleteWindowList.begin(),
                  m_ReserveDeleteWindowList.end(), pKeyCaptureWindow)) {
      m_KeyCaptureWindowMap.insert(
          TKeyCaptureWindowMap::value_type(vkey, pKeyCaptureWindow));
    }
}
