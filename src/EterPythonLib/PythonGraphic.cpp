...
LPDIRECT3D9EX CPythonGraphic::GetD3D() { return ms_lpd3d; }

float CPythonGraphic::GetOrthoDepth() { return m_fOrthoDepth; }
...
void CPythonGraphic::SetViewport(float fx, float fy, float fWidth,
                                 float fHeight) {
  ...
  if (FAILED(ms_lpd3dDevice->SetViewport(&ViewPort))) {
    Tracef("CPythonGraphic::SetViewport(%d, %d, %d, %d) - Error", ViewPort.X,
...
void GenScreenShotTag(const char *src, DWORD crc32, char *leaf,
                      size_t leafLen) {
  const char *p = src;
  const char *n = p;
  while (n = strchr(p, '\\'))
...
bool CPythonGraphic::SaveJPEG(const char *pszFileName, LPBYTE pbyBuffer,
...
bool CPythonGraphic::SaveScreenShot(const char *c_pszFileName) {
  ...
  if (FAILED(hr = ms_lpd3dDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO,
                                                &lpSurface))) {
  ...
  case D3DFMT_A2B10G10R10:
    TraceError("Unsupported BackBuffer Format(%d). Please contact Metin 2 "
               "Administrator.",
               stSurfaceDesc.Format);
    SAFE_RELEASE(lpSurface);
    return false;
  }

  D3DLOCKED_RECT lockRect;
  if (FAILED(hr = lpSurface->LockRect(&lockRect, NULL,
  ...
  if (bSaved == false) {
    TraceError("Failed to save JPEG file. (%s, %d, %d)", c_pszFileName, uWidth,
               uHeight);
    return false;
  }
  ...
  if (g_isScreenShotKey) {
    ...
    if (srcFilePtr) {
      ...
      char imgDesc[64];
      GenScreenShotTag(c_pszFileName, GetCRC32(tail, tailSize), imgDesc,
                       sizeof(imgDesc));
...
void CPythonGraphic::RenderImage(CGraphicImageInstance *pImageInstance, float x,
                                 float y) {
  ...
  RenderTextureBox(x, y, x + width, y + height, 0.0f, 0.5f / width,
                   0.5f / height, (width + 0.5f) / width,
                   (height + 0.5f) / height);
}

void CPythonGraphic::RenderAlphaImage(CGraphicImageInstance *pImageInstance,
                                      float x, float y, float aLeft,
                                      float aRight) {
  assert(pImageInstance != NULL);
...
void CPythonGraphic::RenderCoolTimeBox(float fxCenter, float fyCenter,
  ...
  if (fLastPercentage > 0.0f) {
    ...
    vertex.position.x =
        fxCenter +
        ((pv2Pos->x - pv2LastPos->x) * fLastPercentage + pv2LastPos->x) *
            fRadius;
    vertex.position.y =
        fyCenter +
        ((pv2Pos->y - pv2LastPos->y) * fLastPercentage + pv2LastPos->y) *
            fRadius;
    vertices.push_back(vertex);
    ++iTriCount;
  }
...
CPythonGraphic::~CPythonGraphic() { Tracef("Python Graphic Clear\n"); }
