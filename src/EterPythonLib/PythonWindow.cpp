...
template <typename Func> CWindow *FindWindowUpwards(CWindow *start, Func &f) {
...
CWindow::CWindow(PyObject *ppyObject)
    : m_x(0), m_y(0), m_lWidth(0), m_lHeight(0), m_poHandler(ppyObject),
      m_bShow(false), m_bEnableScissorRect(false), m_pParent(NULL), m_dwFlag(0),
  ...
  m_limitBiasRect.bottom = m_limitBiasRect.left = m_limitBiasRect.right =
      m_limitBiasRect.top = 0;
}
...
struct FClear {
  void operator()(CWindow *pWin) { pWin->Clear(); }
};
...
void CWindow::DestroyHandle() { m_poHandler = NULL; }

void CWindow::Show() { m_bShow = true; }

void CWindow::Hide() { m_bShow = false; }

// NOTE : IsShow는 "자신이 보이는가?" 이지만, __IsShowing은 "자신이 그려지고
// 있는가?" 를 체크한다
//        자신은 Show 지만 Tree 위쪽의 Parent 중 하나는 Hide 일 수 있으므로.. -
//        [levites]
...
void CWindow::__RemoveReserveChildren() {
  ...
  for (it = m_pReserveChildList.begin(); it != m_pReserveChildList.end();
       ++it) {
...
void CWindow::Render() {
  ...
  if (m_bEnableScissorRect) {
    ...
    if (pParentScissorWindow) {
      ...
      parentScissorRect.right =
          parentRect.left + pParentScissorWindow->GetWidth();
      parentScissorRect.bottom =
          parentRect.top + pParentScissorWindow->GetHeight();

      scissorRect.left =
          std::max<int32_t>(scissorRect.left, parentScissorRect.left);
      scissorRect.top =
          std::max<int32_t>(scissorRect.top, parentScissorRect.top);
      scissorRect.right =
          std::min<int32_t>(scissorRect.right, parentScissorRect.right);
      scissorRect.bottom =
          std::min<int32_t>(scissorRect.bottom, parentScissorRect.bottom);
    }
    ...
    if (g_bOutlineBoxEnable) {
      ...
      CPythonGraphic::Instance().RenderBox2d(m_rect.left, m_rect.top,
                                             m_rect.right, m_rect.bottom);
    }

    std::for_each(m_pChildList.begin(), m_pChildList.end(),
                  std::mem_fn(&CWindow::Render));
  } else {
    OnRender();
...
void CWindow::OnUpdate() {
  ...
  PyCallClassMemberFunc_ByPyString(m_poHandler, poFuncName_OnUpdate,
                                   BuildEmptyTuple());
}

void CWindow::EnableScissorRect() { m_bEnableScissorRect = true; }

void CWindow::DisableScissorRect() { m_bEnableScissorRect = false; }

bool CWindow::IsScissorRectEnabled() const { return m_bEnableScissorRect; }
...
void CWindow::SetName(const char *c_szName) { m_strName = c_szName; }
...
long CWindow::UpdateRect() {
  ...
  if (m_pParent) {
    ...
    m_rect.left += 0L < m_pParent->GetWidth()
                       ? m_pParent->m_rect.left
                       : m_pParent->m_rect.right + ::abs(m_pParent->GetWidth());
  }
  m_rect.right = m_rect.left + m_lWidth;
#endif
  std::for_each(m_pChildList.begin(), m_pChildList.end(),
                std::mem_fn(&CWindow::UpdateRect));
...
CWindow *CWindow::GetParent() { return m_pParent; }
...
void CWindow::SetTop(CWindow *pWin) {
  ...
  TWindowContainer::iterator itor =
      std::find(m_pChildList.begin(), m_pChildList.end(), pWin);
...
void CWindow::OnMouseDrag(long lx, long ly) {
  PyCallClassMemberFunc(m_poHandler, "OnMouseDrag",
                        Py_BuildValue("(ii)", lx, ly));
}

void CWindow::OnMoveWindow(long lx, long ly) {
  PyCallClassMemberFunc(m_poHandler, "OnMoveWindow",
                        Py_BuildValue("(ii)", lx, ly));
}
...
BOOL CWindow::OnMouseLeftButtonDown() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnMouseLeftButtonDown",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnMouseLeftButtonDoubleClick() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnMouseLeftButtonDoubleClick",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnMouseRightButtonDown() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnMouseRightButtonDown",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnMouseRightButtonUp() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnMouseRightButtonUp",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnMouseRightButtonDoubleClick() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnMouseRightButtonDoubleClick",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnMouseMiddleButtonDown() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnMouseMiddleButtonDown",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnMouseMiddleButtonUp() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnMouseMiddleButtonUp",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnIMETabEvent() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnIMETab", BuildEmptyTuple(),
                            &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnIMEReturnEvent() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnIMEReturn", BuildEmptyTuple(),
                            &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnIMEKeyDownEvent(int ikey) {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnIMEKeyDown",
                            Py_BuildValue("(i)", ikey), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnIMEOpenCandidateListEvent() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnIMEOpenCandidateList",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnIMECloseCandidateListEvent() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnIMECloseCandidateList",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnIMEOpenReadingWndEvent() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnIMEOpenReadingWnd",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnIMECloseReadingWndEvent() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnIMECloseReadingWnd",
                            BuildEmptyTuple(), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnKeyDown(int ikey) {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnKeyDown",
                            Py_BuildValue("(i)", ikey), &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnKeyUp(int ikey) {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnKeyUp", Py_BuildValue("(i)", ikey),
                            &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnPressEscapeKey() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnPressEscapeKey", BuildEmptyTuple(),
                            &lValue))
    if (0 != lValue)
      return TRUE;
...
BOOL CWindow::OnPressExitKey() {
  long lValue;
  if (PyCallClassMemberFunc(m_poHandler, "OnPressExitKey", BuildEmptyTuple(),
                            &lValue))
    if (0 != lValue)
      return TRUE;
...
void CBar::OnRender() {
  CPythonGraphic::Instance().SetDiffuseColor(m_dwColor);
  CPythonGraphic::Instance().RenderBar2d(m_rect.left, m_rect.top, m_rect.right,
                                         m_rect.bottom);
}
...
void CLine::OnRender() {
  ...
  rkpyGraphic.RenderLine2d(m_rect.left, m_rect.top, m_rect.right,
                           m_rect.bottom);
}
...
void CBar3D::OnRender() {
  ...
  rkpyGraphic.RenderLine2d(m_rect.left, m_rect.bottom, m_rect.right,
                           m_rect.bottom);
  rkpyGraphic.RenderLine2d(m_rect.right, m_rect.top, m_rect.right,
                           m_rect.bottom);
}
...
CTextLine::CTextLine(PyObject *ppyObject) : CWindow(ppyObject) {
  ...
  m_baseDirection = ::IsRTL() ? CGraphicTextInstance::ETextDirection::RTL
                              : CGraphicTextInstance::ETextDirection::LTR;
  m_TextInstance.SetTextDirection(m_baseDirection);
}
CTextLine::~CTextLine() { m_TextInstance.Destroy(); }

void CTextLine::SetMax(int iMax) { m_TextInstance.SetMax(iMax); }
...
void CTextLine::SetText(const char *c_szText) { OnSetText(c_szText); }
...
void CTextLine::ShowCursor() { m_TextInstance.ShowCursor(); }
void CTextLine::HideCursor() { m_TextInstance.HideCursor(); }
...
void CTextLine::OnChangePosition() {
  ...
  m_TextInstance.SetPosition(bAnchorRTL ? m_rect.right : m_rect.left,
                             m_rect.top);
}
...
CImageBox::~CImageBox() { OnDestroyInstance(); }
...
BOOL CImageBox::LoadImage(const char *c_szFileName) {
  ...
  CResource *pResource =
      CResourceManager::Instance().GetResourcePointer(c_szFileName);
  if (!pResource)
    return FALSE;
...
DWORD CExpandedImageBox::Type() {
  static DWORD s_dwType =
      GetCRC32("CExpandedImageBox", strlen("CExpandedImageBox"));
  return (s_dwType);
}
...
CExpandedImageBox::CExpandedImageBox(PyObject *ppyObject)
    : CImageBox(ppyObject) {}
...
void CExpandedImageBox::OnDestroyInstance() {
  if (m_pImageInstance) {
    CGraphicExpandedImageInstance::Delete(
        (CGraphicExpandedImageInstance *)m_pImageInstance);
    m_pImageInstance = NULL;
  }
}
...
void CExpandedImageBox::SetRenderingRect(float fLeft, float fTop, float fRight,
                                         float fBottom) {
  if (!m_pImageInstance)
    return;
...
CAniImageBox::CAniImageBox(PyObject *ppyObject)
    : CWindow(ppyObject), m_bycurDelay(0), m_byDelay(4), m_bycurIndex(0) {
  m_ImageVector.clear();
}
CAniImageBox::~CAniImageBox() {
  for_each(m_ImageVector.begin(), m_ImageVector.end(),
           CGraphicExpandedImageInstance::DeleteExpandedImageInstance);
}
...
void CAniImageBox::AppendImage(const char *c_szFileName) {
  ...
  //		SetSize(pImageInstance->GetWidth(),
  // pImageInstance->GetHeight()); 		UpdateRect();
}
...
void CAniImageBox::SetRenderingRect(float fLeft, float fTop, float fRight,
                                    float fBottom) {
...
CButton::CButton(PyObject *ppyObject)
    : CWindow(ppyObject), m_pcurVisual(NULL), m_bEnable(TRUE),
      m_isPressed(FALSE), m_isFlash(FALSE) {
...
BOOL CButton::IsEnable() { return m_bEnable; }

BOOL CButton::IsPressed() { return m_isPressed; }
...
void CDragButton::SetRestrictMovementArea(int ix, int iy, int iwidth,
                                          int iheight) {
