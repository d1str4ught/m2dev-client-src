...
//
// Functions for loading a DDS texture and creating a Direct3D runtime resource
// for it
//
// Note these functions are useful as a light-weight runtime loader for DDS
// files. For a full-featured DDS file reader, writer, and texture processing
// pipeline see the 'Texconv' sample and the 'DirectXTex' library.
//
...
//--------------------------------------------------------------------------------------
HRESULT LoadTextureDataFromMemory(_In_reads_(ddsDataSize)
                                      const uint8_t *ddsData,
                                  size_t ddsDataSize, const DDS_HEADER **header,
...
HRESULT LoadTextureDataFromFile(_In_z_ const wchar_t *fileName,
  ...
  *bitSize = 0;
...
#else
  ScopedHandle hFile(
      safe_handle(CreateFileW(fileName, GENERIC_READ, FILE_SHARE_READ, nullptr,
  ...
  if (!GetFileInformationByHandleEx(hFile.get(), FileStandardInfo, &fileInfo,
                                    sizeof(fileInfo))) {
    return HRESULT_FROM_WIN32(GetLastError());
  }
  ...
  // Need at least enough data to fill the header and magic number to be a valid
  // DDS
  ...
  // read the data in
  DWORD bytesRead = 0;
  if (!ReadFile(hFile.get(), ddsData.get(), fileInfo.EndOfFile.LowPart,
                &bytesRead, nullptr)) {
    ddsData.reset();
    return HRESULT_FROM_WIN32(GetLastError());
  }
  ...
  auto hdr =
      reinterpret_cast<const DDS_HEADER *>(ddsData.get() + sizeof(uint32_t));
...
size_t BitsPerPixel(_In_ D3DFORMAT fmt) noexcept {
  ...
  case D3DFMT_YUY2:
    ...
    // (note how it says that D3DFMT_R8G8_B8G8 is "A 16-bit packed RGB format
    // analogous to UYVY (U0Y0, V0Y1, U2Y2, and so on)")
  case D3DFMT_UYVY:
    return 16;
...
//--------------------------------------------------------------------------------------
HRESULT GetSurfaceInfo(_In_ size_t width, _In_ size_t height,
                       _In_ D3DFORMAT fmt, size_t *outNumBytes,
...
D3DFORMAT GetD3D9Format(const DDS_PIXELFORMAT &ddpf) noexcept {
  if (ddpf.flags & DDS_RGB) {
    ...
    case 32:
      ...
      // For 'correct' writers this should be 0x3ff00000,0x000ffc00,0x000003ff
      // for BGR data
      if (ISBITMASK(0x000003ff, 0x000ffc00, 0x3ff00000, 0xc0000000)) {
        return D3DFMT_A2R10G10B10;
      }

      // For 'correct' writers this should be 0x000003ff,0x000ffc00,0x3ff00000
      // for RGB data
    ...
    case 8:
      ...
      // Paletted texture formats are typically not supported on modern video
      // cards aka D3DFMT_P8, D3DFMT_A8P8
      break;
    }
    ...
    case 8:
      ...
      if (ISBITMASK(0x00ff, 0, 0, 0xff00)) {
        return D3DFMT_A8L8; // Some DDS writers assume the bitcount should be 8
                            // instead of 16
      }
      break;
    }
...
HRESULT CreateTextureFromDDS(_In_ LPDIRECT3DDEVICE9 device,
                             ...
                             _In_ size_t bitSize, _In_ DWORD usage,
                             _In_ D3DPOOL pool,
  ...
  // Bound sizes (for security purposes we don't trust DDS file metadata larger
  // than the D3D 10 hardware requirements)
  if (iMipCount > 14u /*D3D10_REQ_MIP_LEVELS*/) {
    return HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED);
  }

  // We could support a subset of 'DX10' extended header DDS files, but we'll
  // assume here we are only supporting legacy DDS files for a Direct3D9 device
  ...
  if (header->flags & DDS_HEADER_FLAGS_VOLUME) {
    ...
    hr = device->CreateVolumeTexture(iWidth, iHeight, iDepth, iMipCount, usage,
                                     fmt, pool, pTexture.GetAddressOf(),
                                     nullptr);
    if (FAILED(hr))
      return hr;
    ...
    if (pool == D3DPOOL_DEFAULT) {
      hr = device->CreateVolumeTexture(iWidth, iHeight, iDepth, iMipCount, 0u,
                                       fmt, D3DPOOL_SYSTEMMEM,
      ...
      if (SUCCEEDED(pStagingTexture->LockBox(i, &LockedBox, nullptr, 0))) {
        ...
        for (UINT j = 0; j < iDepth; ++j) {
          ...
          for (size_t h = 0; h < NumRows; h++) {
            memcpy_s(dptr, static_cast<size_t>(LockedBox.RowPitch), sptr,
                     RowBytes);
    ...
    *texture = pTexture.Detach();
    ...
    hr = device->CreateCubeTexture(iWidth, iMipCount, usage, fmt, pool,
    ...
    if (pool == D3DPOOL_DEFAULT) {
      hr = device->CreateCubeTexture(iWidth, iMipCount, 0u, fmt,
                                     D3DPOOL_SYSTEMMEM,
                                     pStagingTexture.GetAddressOf(), nullptr);
      if (FAILED(hr))
        return hr;
    ...
    for (UINT f = 0; f < 6; ++f, mask <<= 1) {
      ...
      for (UINT i = 0; i < iMipCount; ++i) {
        ...
        if (SUCCEEDED(
                pStagingTexture->LockRect(static_cast<D3DCUBEMAP_FACES>(f), i,
                                          &LockedRect, nullptr, 0))) {
          auto pDestBits = static_cast<uint8_t *>(LockedRect.pBits);

          // Copy stride line by line
          for (size_t r = 0; r < NumRows; r++) {
            memcpy_s(pDestBits, static_cast<size_t>(LockedRect.Pitch), pSrcBits,
                     RowBytes);
    ...
    *texture = pTexture.Detach();
    ...
    hr = device->CreateTexture(iWidth, iHeight, iMipCount, usage, fmt, pool,
                               pTexture.GetAddressOf(), nullptr);
    if (FAILED(hr))
      return hr;
    ...
    if (pool == D3DPOOL_DEFAULT) {
      hr = device->CreateTexture(iWidth, iHeight, iMipCount, 0u, fmt,
                                 D3DPOOL_SYSTEMMEM,
                                 pStagingTexture.GetAddressOf(), nullptr);
      if (FAILED(hr))
        return hr;
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemory(
    LPDIRECT3DDEVICE9 d3dDevice, const uint8_t *ddsData, size_t ddsDataSize,
    LPDIRECT3DBASETEXTURE9 *texture, bool generateMipsIfMissing) noexcept {
  return CreateDDSTextureFromMemoryEx(d3dDevice, ddsData, ddsDataSize, 0u,
                                      D3DPOOL_DEFAULT, generateMipsIfMissing,
                                      texture);
}

_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemoryEx(
    LPDIRECT3DDEVICE9 d3dDevice, const uint8_t *ddsData, size_t ddsDataSize,
    _In_ DWORD usage, _In_ D3DPOOL pool, bool generateMipsIfMissing,
  ...
  HRESULT hr = LoadTextureDataFromMemory(ddsData, ddsDataSize, &header,
                                         &bitData, &bitSize);
  if (FAILED(hr))
    return hr;

  return CreateTextureFromDDS(d3dDevice, header, bitData, bitSize, usage, pool,
                              texture, generateMipsIfMissing);
}

// Type-specific standard versions
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemory(
    LPDIRECT3DDEVICE9 d3dDevice, const uint8_t *ddsData, size_t ddsDataSize,
    LPDIRECT3DTEXTURE9 *texture, bool generateMipsIfMissing) noexcept {
  if (texture) {
    *texture = nullptr;
  }
  ...
  HRESULT hr = CreateDDSTextureFromMemoryEx(
      d3dDevice, ddsData, ddsDataSize, 0u, D3DPOOL_DEFAULT,
      generateMipsIfMissing, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemory(
  ...
  ComPtr<IDirect3DBaseTexture9> tex;
  HRESULT hr =
      CreateDDSTextureFromMemoryEx(d3dDevice, ddsData, ddsDataSize, 0u,
                                   D3DPOOL_DEFAULT, false, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemoryEx(
    LPDIRECT3DDEVICE9 d3dDevice, const uint8_t *ddsData, size_t ddsDataSize,
    DWORD usage, D3DPOOL pool, bool generateMipsIfMissing,
  ...
  ComPtr<IDirect3DBaseTexture9> tex;
  HRESULT hr =
      CreateDDSTextureFromMemoryEx(d3dDevice, ddsData, ddsDataSize, usage, pool,
                                   generateMipsIfMissing, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
    if (tex->GetType() == D3DRTYPE_TEXTURE) {
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemoryEx(
    LPDIRECT3DDEVICE9 d3dDevice, const uint8_t *ddsData, size_t ddsDataSize,
    DWORD usage, D3DPOOL pool, LPDIRECT3DCUBETEXTURE9 *texture) noexcept {
  if (texture) {
    *texture = nullptr;
  }
  ...
  ComPtr<IDirect3DBaseTexture9> tex;
  HRESULT hr = CreateDDSTextureFromMemoryEx(
      d3dDevice, ddsData, ddsDataSize, usage, pool, false, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
    if (tex->GetType() == D3DRTYPE_CUBETEXTURE) {
      *texture = static_cast<LPDIRECT3DCUBETEXTURE9>(tex.Detach());
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemoryEx(
    LPDIRECT3DDEVICE9 d3dDevice, const uint8_t *ddsData, size_t ddsDataSize,
    DWORD usage, D3DPOOL pool, LPDIRECT3DVOLUMETEXTURE9 *texture) noexcept {
  if (texture) {
    *texture = nullptr;
  }
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFile(
    LPDIRECT3DDEVICE9 d3dDevice, const wchar_t *fileName,
    LPDIRECT3DBASETEXTURE9 *texture, bool generateMipsIfMissing) noexcept {
  return CreateDDSTextureFromFileEx(d3dDevice, fileName, 0u, D3DPOOL_DEFAULT,
                                    generateMipsIfMissing, texture);
}

_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFileEx(
    LPDIRECT3DDEVICE9 d3dDevice, const wchar_t *fileName, _In_ DWORD usage,
    _In_ D3DPOOL pool, bool generateMipsIfMissing,
    LPDIRECT3DBASETEXTURE9 *texture) noexcept {
  if (texture) {
    *texture = nullptr;
  }
  ...
  std::unique_ptr<uint8_t[]> ddsData;
  HRESULT hr =
      LoadTextureDataFromFile(fileName, ddsData, &header, &bitData, &bitSize);
  if (FAILED(hr)) {
    return hr;
  }
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFile(
  ...
  ComPtr<IDirect3DBaseTexture9> tex;
  HRESULT hr =
      CreateDDSTextureFromFileEx(d3dDevice, fileName, 0u, D3DPOOL_DEFAULT,
                                 generateMipsIfMissing, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
    if (tex->GetType() == D3DRTYPE_TEXTURE) {
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFile(
  ...
  HRESULT hr = CreateDDSTextureFromFileEx(
      d3dDevice, fileName, 0u, D3DPOOL_DEFAULT, false, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
    if (tex->GetType() == D3DRTYPE_CUBETEXTURE) {
      *texture = static_cast<LPDIRECT3DCUBETEXTURE9>(tex.Detach());
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFile(
    LPDIRECT3DDEVICE9 d3dDevice, const wchar_t *szFileName,
    LPDIRECT3DVOLUMETEXTURE9 *texture) noexcept {
  if (texture) {
    *texture = nullptr;
  }
  ...
  ComPtr<IDirect3DBaseTexture9> tex;
  HRESULT hr = CreateDDSTextureFromFileEx(
      d3dDevice, szFileName, 0u, D3DPOOL_DEFAULT, false, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
    if (tex->GetType() == D3DRTYPE_VOLUMETEXTURE) {
      *texture = static_cast<LPDIRECT3DVOLUMETEXTURE9>(tex.Detach());
...
// Type-specific extended versions
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFileEx(
    LPDIRECT3DDEVICE9 d3dDevice, const wchar_t *fileName, DWORD usage,
    D3DPOOL pool, bool generateMipsIfMissing,
    LPDIRECT3DTEXTURE9 *texture) noexcept {
  if (texture) {
    *texture = nullptr;
  }
  ...
  ComPtr<IDirect3DBaseTexture9> tex;
  HRESULT hr =
      CreateDDSTextureFromFileEx(d3dDevice, fileName, usage, pool,
                                 generateMipsIfMissing, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
    if (tex->GetType() == D3DRTYPE_TEXTURE) {
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFileEx(
    LPDIRECT3DDEVICE9 d3dDevice, const wchar_t *fileName, DWORD usage,
    D3DPOOL pool, LPDIRECT3DCUBETEXTURE9 *texture) noexcept {
  if (texture) {
    *texture = nullptr;
  }
  ...
  ComPtr<IDirect3DBaseTexture9> tex;
  HRESULT hr = CreateDDSTextureFromFileEx(d3dDevice, fileName, usage, pool,
                                          false, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
    if (tex->GetType() == D3DRTYPE_CUBETEXTURE) {
      *texture = static_cast<LPDIRECT3DCUBETEXTURE9>(tex.Detach());
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFileEx(
    LPDIRECT3DDEVICE9 d3dDevice, const wchar_t *szFileName, DWORD usage,
    D3DPOOL pool, LPDIRECT3DVOLUMETEXTURE9 *texture) noexcept {
  if (texture) {
    *texture = nullptr;
  }
  ...
  ComPtr<IDirect3DBaseTexture9> tex;
  HRESULT hr = CreateDDSTextureFromFileEx(d3dDevice, szFileName, usage, pool,
                                          false, tex.GetAddressOf());
  if (SUCCEEDED(hr)) {
    hr = E_FAIL;
    if (tex->GetType() == D3DRTYPE_VOLUMETEXTURE) {
