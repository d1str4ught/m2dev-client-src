...
static CGraphicVertexBuffer *
__AllocDeformVertexBuffer(unsigned deformableVertexCount) {
  ...
  // TraceError("NEW %8d: %d(%d)", time(NULL) - base, capacity,
  // deformableVertexCount);

  CGraphicVertexBuffer *pkNewVB = new CGraphicVertexBuffer;

  if (!pkNewVB->Create(capacity, D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_TEX1,
                       D3DUSAGE_DYNAMIC, D3DPOOL_DEFAULT)) {
    TraceError("NEW_ERROR %8d: %d(%d)", time(NULL) - base, capacity,
               deformableVertexCount);
  }
...
void __ReserveSharedVertexBuffers(unsigned index, unsigned count) {
  ...
  for (unsigned i = 0; i != count; ++i) {
    CGraphicVertexBuffer *pkNewVB = new CGraphicVertexBuffer;
    pkNewVB->Create(capacity, D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_TEX1,
                    D3DUSAGE_DYNAMIC, D3DPOOL_DEFAULT);
...
void CGrannyLODController::SetMaterialImagePointer(const char *c_szImageName,
...
void CGrannyLODController::SetMaterialData(
    const char *c_szImageName, const SMaterialData &c_rkMaterialData) {
  std::deque<CGrannyModelInstance *>::iterator i;
  for (i = m_que_pkModelInst.begin(); i != m_que_pkModelInst.end(); ++i) {
    CGrannyModelInstance *pkModelInst = (*i);
...
void CGrannyLODController::SetSpecularInfo(const char *c_szMtrlName,
...
CGrannyLODController::CGrannyLODController()
    : m_pCurrentModelInstance(NULL), m_bLODLevel(0),
      m_pAttachedParentModel(NULL), m_fLODDistance(0.0f),
...
void CGrannyLODController::Clear() {
  ...
  std::for_each(m_que_pkModelInst.begin(), m_que_pkModelInst.end(),
                CGrannyModelInstance::Delete);
  m_que_pkModelInst.clear();

  std::vector<TAttachingModelData>::iterator itor =
      m_AttachedModelDataVector.begin();
...
void CGrannyLODController::AddModel(CGraphicThing *pThing, int iSrcModel,
                                    CGrannyLODController *pSkelLODController) {
  if (!pThing)
    return;
  ...
  if (pSkelLODController) {
    pModelInstance->SetLinkedModelPointer(
        pModel, m_pkSharedDeformableVertexBuffer,
        &pSkelLODController->m_pCurrentModelInstance);
  } else {
    pModelInstance->SetLinkedModelPointer(
        pModel, m_pkSharedDeformableVertexBuffer, NULL);
  }
  ...
  if (!m_pCurrentModelInstance) {
    ...
    //         Deform을 하지 않으면 NULL 입니다. 구조가 조금 바뀌어야 할지도.. -
    //         [levites]
    pModelInstance->DeformNoSkin(&ms_matIdentity);
  }
...
void CGrannyLODController::__ReserveSharedDeformableVertexBuffer(
    DWORD deformableVertexCount) {
  if (m_pkSharedDeformableVertexBuffer &&
      m_pkSharedDeformableVertexBuffer->GetVertexCount() >=
          deformableVertexCount)
    return;

  __FreeDeformVertexBuffer(m_pkSharedDeformableVertexBuffer);

  m_pkSharedDeformableVertexBuffer =
      __AllocDeformVertexBuffer(deformableVertexCount);
}

void CGrannyLODController::AttachModelInstance(
    CGrannyLODController *pSrcLODController, const char *c_szBoneName) {
...
void CGrannyLODController::DetachModelInstance(
...
#ifdef CHECK_LOD
  if (m_que_pkModelInst.size() > 0 &&
      m_pCurrentModelInstance == m_que_pkModelInst[0])
    m_pCurrentModelInstance->RenderWithoutTexture();

  if (m_que_pkModelInst.size() > 1 &&
      m_pCurrentModelInstance == m_que_pkModelInst[1])
    m_pCurrentModelInstance->RenderWithOneTexture();

  if (m_que_pkModelInst.size() > 2 &&
      m_pCurrentModelInstance == m_que_pkModelInst[2])
    m_pCurrentModelInstance->RenderWithOneTexture();

  if (m_que_pkModelInst.size() > 3 &&
      m_pCurrentModelInstance == m_que_pkModelInst[3])
    m_pCurrentModelInstance->RenderWithOneTexture();
...
void CGrannyLODController::Update(float fElapsedTime, float fDistanceFromCenter,
                                  float fDistanceFromCamera) {
  UpdateLODLevel(fDistanceFromCenter, fDistanceFromCamera);
  UpdateTime(fElapsedTime);
}

void CGrannyLODController::UpdateLODLevel(float fDistanceFromCenter,
                                          float fDistanceFromCamera) {
  if (m_que_pkModelInst.size() <= 1)
    return;
  ...
  if (fDistanceFromCenter > LOD_APPLY_MIN) // 중심 LOD 예외 취소
  {
    ...
    float fLODFactor =
        fMINMAX(0.0f, (m_fLODDistance - fDistanceFromCamera), m_fLODDistance);

    if (m_fLODDistance > 0.0f)
      m_dwLODAniFPS =
          (DWORD)((CGrannyModelInstance::ANIFPS_MAX -
                   CGrannyModelInstance::ANIFPS_MIN) *
                      fLODFactor / m_fLODDistance +
                  static_cast<float>(CGrannyModelInstance::ANIFPS_MIN));
    else
    ...
    if (m_fLODDistance <= 5000.0f) {
      ...
      bLODLevel =
          (BYTE)(m_que_pkModelInst.size() -
                 std::min((size_t)bLODLevel, m_que_pkModelInst.size()) - 1);
    }
...
void CGrannyLODController::SetCurrentModelInstance(
    CGrannyModelInstance *pgrnModelInstance) {
  // Copy Motion
...
void CGrannyLODController::RefreshAttachedModelInstance() {
  ...
  for (DWORD i = 0; i < m_AttachedModelDataVector.size(); ++i) {
    ...
    CGrannyModelInstance *pSrcInstance =
        rModelData.pkLODController->GetModelInstance();
    if (!pSrcInstance) {
      Tracenf("CGrannyLODController::RefreshAttachedModelInstance : "
              "m_AttachedModelDataVector[%d]->pkLODController->GetModelIntance("
              ")==NULL",
              i);
      continue;
    }

    pSrcInstance->SetParentModelInstance(m_pCurrentModelInstance,
                                         rModelData.strBoneName.c_str());
  }
}

void CGrannyLODController::UpdateSkeleton(const D3DXMATRIX *c_pWorldMatrix,
                                          float fElapsedTime) {
  if (m_pCurrentModelInstance)
...
bool CGrannyLODController::Intersect(const D3DXMATRIX *c_pMatrix, float *u,
                                     float *v, float *t) {
  if (!m_pCurrentModelInstance)
    return false;
...
void CGrannyLODController::SetMotionPointer(const CGrannyMotion *c_pMotion,
                                            float fBlendTime, int iLoopCount,
                                            float speedRatio) {
  assert(m_pCurrentModelInstance != NULL);
  m_pCurrentModelInstance->SetMotionPointer(c_pMotion, fBlendTime, iLoopCount,
                                            speedRatio);
}

void CGrannyLODController::ChangeMotionPointer(const CGrannyMotion *c_pMotion,
                                               int iLoopCount,
                                               float speedRatio) {
  assert(m_pCurrentModelInstance != NULL);
  m_pCurrentModelInstance->ChangeMotionPointer(c_pMotion, iLoopCount,
                                               speedRatio);
}
