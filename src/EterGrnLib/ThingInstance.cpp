...
bool CGraphicThingInstance::LessRenderOrder(
...
void CGraphicThingInstance::DestroySystem() { ms_kPool.Destroy(); }

CGraphicThingInstance *CGraphicThingInstance::New() { return ms_kPool.Alloc(); }
...
void CGraphicThingInstance::SetMotionAtEnd() {
  std::for_each(m_LODControllerVector.begin(), m_LODControllerVector.end(),
                std::mem_fn(&CGrannyLODController::SetMotionAtEnd));
}

bool CGraphicThingInstance::Picking(const D3DXVECTOR3 &v,
                                    const D3DXVECTOR3 &dir, float &out_x,
                                    float &out_y) {
...
void CGraphicThingInstance::OnUpdateCollisionData(
...
void CGraphicThingInstance::OnUpdateHeighInstance(
    CAttributeInstance *pAttributeInstance) {
  assert(pAttributeInstance);
  SetHeightInstance(pAttributeInstance);
}

bool CGraphicThingInstance::OnGetObjectHeight(float fX, float fY,
                                              float *pfHeight) {
  if (m_pHeightAttributeInstance &&
      m_pHeightAttributeInstance->GetHeight(fX, fY, pfHeight))
    return true;
  return false;
}
...
void CGraphicThingInstance::BuildBoundingSphere() {
  ...
  m_fRadius = D3DXVec3Length(&vDelta) * 0.5f +
              50.0f; // extra length for attached objects
}

bool CGraphicThingInstance::GetBoundingSphere(D3DXVECTOR3 &v3Center,
                                              float &fRadius) {
  if (m_fRadius <= 0) {
    BuildBoundingSphere();
...
bool CGraphicThingInstance::GetBoundingAABB(D3DXVECTOR3 &v3Min,
                                            D3DXVECTOR3 &v3Max) {
  BuildBoundingAABB();
...
bool CGraphicThingInstance::CheckMotionThingIndex(DWORD dwMotionKey) {
  std::map<DWORD, CGraphicThing::TRef *>::iterator itor =
      m_roMotionThingMap.find(dwMotionKey);
...
bool CGraphicThingInstance::GetMotionThingPointer(DWORD dwKey,
                                                  CGraphicThing **ppMotion) {
...
bool CGraphicThingInstance::FindBoneIndex(int iModelInstance,
                                          const char *c_szBoneName,
                                          int *iRetBone) {
  assert(CheckModelInstanceIndex(iModelInstance));

  CGrannyModelInstance *pModelInstance =
      m_LODControllerVector[iModelInstance]->GetModelInstance();
...
void CGraphicThingInstance::AttachModelInstance(int iDstModelInstance,
                                                const char *c_szBoneName,
                                                int iSrcModelInstance) {
  if (!CheckModelInstanceIndex(iSrcModelInstance)) {
    TraceError("CGraphicThingInstance::AttachModelInstance(iDstModelInstance=%"
               "d, c_szBoneName=%s, iSrcModelInstance=%d)",
  ...
  CGrannyLODController *pSrcLODController =
      m_LODControllerVector[iSrcModelInstance];
  CGrannyLODController *pDstLODController =
      m_LODControllerVector[iDstModelInstance];
  pDstLODController->AttachModelInstance(pSrcLODController, c_szBoneName);
}

void CGraphicThingInstance::AttachModelInstance(
    int iDstModelInstance, const char *c_szBoneName,
    CGraphicThingInstance &rSrcInstance, int iSrcModelInstance) {
  if (!CheckModelInstanceIndex(iDstModelInstance)) {
    TraceError("CGraphicThingInstance::AttachModelInstance(iDstModelInstance=%"
               "d, c_szBoneName=%s, iSrcModelInstance=%d)",
               iDstModelInstance, c_szBoneName, iSrcModelInstance);
  ...
  CGrannyLODController *pDstLODController =
      m_LODControllerVector[iDstModelInstance];
  CGrannyLODController *pSrcLODController =
      rSrcInstance.m_LODControllerVector[iSrcModelInstance];
  pDstLODController->AttachModelInstance(pSrcLODController, c_szBoneName);
}

void CGraphicThingInstance::DetachModelInstance(
    int iDstModelInstance, CGraphicThingInstance &rSrcInstance,
    int iSrcModelInstance) {
  if (!CheckModelInstanceIndex(iDstModelInstance)) {
    TraceError("CGraphicThingInstance::AttachModelInstance(iDstModelInstance=%"
               "d, iSrcModelInstance=%d)",
               iDstModelInstance, iSrcModelInstance);
    return;
  }
  if (!rSrcInstance.CheckModelInstanceIndex(iSrcModelInstance)) {
    TraceError("CGraphicThingInstance::AttachModelInstance(iDstModelInstance=%"
...
bool CGraphicThingInstance::GetBonePosition(int iModelIndex, int iBoneIndex,
                                            float *pfx, float *pfy,
                                            float *pfz) {
  assert(CheckModelInstanceIndex(iModelIndex));

  CGrannyModelInstance *pModelInstance =
      m_LODControllerVector[iModelIndex]->GetModelInstance();
...
bool CGraphicThingInstance::SetModelInstance(int iDstModelInstance,
                                             int iSrcModelThing, int iSrcModel,
                                             int iSkelInstance) {
  if (!CheckModelInstanceIndex(iDstModelInstance)) {
    TraceError("CGraphicThingInstance::SetModelInstance(iDstModelInstance=%d, "
  ...
  if (iSkelInstance != DONTUSEVALUE) {
    if (!CheckModelInstanceIndex(iSkelInstance)) {
      TraceError("CGraphicThingInstance::"
                 "SetModelInstanceByOtherSkeletonInstance(iSkelInstance=%d, "
                 "iDstModelInstance=%d, pModelThing=%d, iSrcModel=%d)\n",
  ...
  for (DWORD i = 0; i < rModelThingSet.m_pLODThingRefVector.size(); ++i) {
    ...
    pController->AddModel(rModelThingSet.m_pLODThingRefVector[i]->GetPointer(),
                          iSrcModel, pSkelController);
  }
  return true;
}

void CGraphicThingInstance::SetMaterialImagePointer(UINT ePart,
                                                    const char *c_szImageName,
                                                    CGraphicImage *pImage) {
  if (ePart >= m_LODControllerVector.size()) {
    TraceError(
        "CGraphicThingInstance::SetMaterialImagePointer(ePart(%d)<uPartCount(%"
        "d), c_szImageName=%s, pImage=%s) - ePart OUT OF RANGE",
        ePart, m_LODControllerVector.size(), c_szImageName,
        pImage->GetFileName());
  ...
  if (!m_LODControllerVector[ePart]) {
    TraceError("CGraphicThingInstance::SetMaterialImagePointer(ePart(%d), "
               "c_szImageName=%s, pImage=%s) - ePart Data is NULL",
               ePart, m_LODControllerVector.size(), c_szImageName,
               pImage->GetFileName());
...
void CGraphicThingInstance::SetMaterialData(UINT ePart,
                                            const char *c_szImageName,
                                            SMaterialData kMaterialData) {
  if (ePart >= m_LODControllerVector.size()) {
    TraceError("CGraphicThingInstance::SetMaterialData(ePart(%d)<uPartCount(%d)"
               ") - ePart OUT OF RANGE",
  ...
  if (!m_LODControllerVector[ePart]) {
    TraceError("CGraphicThingInstance::SetMaterialData(ePart(%d)) - ePart Data "
               "is NULL",
               ePart, m_LODControllerVector.size());
...
void CGraphicThingInstance::SetSpecularInfo(UINT ePart,
                                            const char *c_szMtrlName,
                                            BOOL bEnable, float fPower) {
  if (ePart >= m_LODControllerVector.size()) {
    TraceError("CGraphicThingInstance::SetSpecularInfo(ePart(%d)<uPartCount(%d)"
               ") - ePart OUT OF RANGE",
               ePart, m_LODControllerVector.size());
  ...
  if (!m_LODControllerVector[ePart]) {
    TraceError("CGraphicThingInstance::SetSpecularInfo(ePart(%d)) - ePart Data "
               "is NULL",
               ePart, m_LODControllerVector.size());
...
bool CGraphicThingInstance::SetMotion(DWORD dwMotionKey, float blendTime,
                                      int loopCount, float speedRatio) {
...
bool CGraphicThingInstance::ChangeMotion(DWORD dwMotionKey, int loopCount,
                                         float speedRatio) {
  if (!CheckMotionThingIndex(dwMotionKey))
    return false;
...
void CGraphicThingInstance::SetEndStopMotion() {
  std::for_each(m_LODControllerVector.begin(), m_LODControllerVector.end(),
                CGrannyLODController::FEndStopMotionPointer());
}

void CGraphicThingInstance::RegisterModelThing(int iModelThing,
                                               CGraphicThing *pModelThing) {
  if (!CheckModelThingIndex(iModelThing)) {
    TraceError("CGraphicThingInstance::RegisterModelThing(iModelThing=%d, "
               "pModelThing=%s)\n",
...
void CGraphicThingInstance::RegisterLODThing(int iModelThing,
                                             CGraphicThing *pModelThing) {
...
void CGraphicThingInstance::RegisterMotionThing(DWORD dwMotionKey,
                                                CGraphicThing *pMotionThing) {
  CGraphicThing::TRef *pMotionRef = new CGraphicThing::TRef;
  pMotionRef->SetPointer(pMotionThing);
  m_roMotionThingMap.insert(std::map<DWORD, CGraphicThing::TRef *>::value_type(
      dwMotionKey, pMotionRef));
}
...
*/
void CGraphicThingInstance::InsertDelay(float fDelay) { m_fDelay = fDelay; }

float CGraphicThingInstance::GetLastLocalTime() { return m_fLastLocalTime; }

float CGraphicThingInstance::GetLocalTime() { return m_fLocalTime; }

float CGraphicThingInstance::GetSecondElapsed() { return m_fSecondElapsed; }
...
bool CGraphicThingInstance::Intersect(float *pu, float *pv, float *pt) {
  ...
  if (m_LODControllerVector.empty()) {
    // TraceError("CGraphicThingInstance::Intersect - m_LODControllerVector is
    // empty");
    return false;
  }
...
void CGraphicThingInstance::GetBoundBox(D3DXVECTOR3 *vtMin,
                                        D3DXVECTOR3 *vtMax) {
  vtMin->x = vtMin->y = vtMin->z = 100000.0f;
  vtMax->x = vtMax->y = vtMax->z = -100000.0f;
  std::for_each(m_LODControllerVector.begin(), m_LODControllerVector.end(),
                CGrannyLODController::FBoundBox(vtMin, vtMax));
}

BOOL CGraphicThingInstance::GetBoundBox(DWORD dwModelInstanceIndex,
                                        D3DXVECTOR3 *vtMin,
                                        D3DXVECTOR3 *vtMax) {
  ...
  CGrannyLODController *pController =
      m_LODControllerVector[dwModelInstanceIndex];
...
BOOL CGraphicThingInstance::GetBoneMatrix(DWORD dwModelInstanceIndex,
                                          DWORD dwBoneIndex,
                                          D3DXMATRIX **ppMatrix) {
  if (!CheckModelInstanceIndex(dwModelInstanceIndex))
    return FALSE;
  ...
  CGrannyModelInstance *pModelInstance =
      m_LODControllerVector[dwModelInstanceIndex]->GetModelInstance();
  if (!pModelInstance)
    return FALSE;
...
BOOL CGraphicThingInstance::GetCompositeBoneMatrix(DWORD dwModelInstanceIndex,
                                                   DWORD dwBoneIndex,
                                                   D3DXMATRIX **ppMatrix) {
  if (!CheckModelInstanceIndex(dwModelInstanceIndex))
    return FALSE;
  ...
  CGrannyModelInstance *pModelInstance =
      m_LODControllerVector[dwModelInstanceIndex]->GetModelInstance();
  if (!pModelInstance) {
    // TraceError("CGraphicThingInstance::GetCompositeBoneMatrix(dwModelInstanceIndex=%d,
    // dwBoneIndex=%d, D3DXMATRIX ** ppMatrix)", dwModelInstanceIndex,
    // dwBoneIndex);
    return FALSE;
  }

  *ppMatrix =
      (D3DXMATRIX *)pModelInstance->GetCompositeBoneMatrixPointer(dwBoneIndex);

  return TRUE;
}

void CGraphicThingInstance::UpdateTransform(D3DXMATRIX *pMatrix,
                                            float fSecondsElapsed,
                                            int iModelInstanceIndex) {
  // TraceError("%s",GetBaseThingPtr()->GetFileName());
  int nLODCount = m_LODControllerVector.size();
  if (iModelInstanceIndex >= nLODCount) {
    // TraceError("void CGraphicThingInstance::UpdateTransform(pMatrix,
    // fSecondsElapsed=%f, iModelInstanceIndex=%d/nLODCount=%d)",
  ...
  if (!pkLODCtrl) {
    // TraceError("void CGraphicThingInstance::UpdateTransform(pMatrix,
    // fSecondsElapsed=%f, iModelInstanceIndex=%d/nLODCount=%d) -
    // m_LODControllerVector[iModelInstanceIndex] == NULL", 	fSecondsElapsed,
    // iModelInstanceIndex, nLODCount);
    return;
  }
  ...
  if (!pModelInstance) {
    /*	TraceError("void CGraphicThingInstance::UpdateTransform(pMatrix,
       fSecondsElapsed=%f, iModelInstanceIndex=%d/nLODCount=%d) -
       pkLODCtrl->GetModelInstance() == NULL", fSecondsElapsed,
       iModelInstanceIndex, nLODCount);*/
    return;
  }
...
void CGraphicThingInstance::DeformNoSkin() {
  ...
  for (i = m_LODControllerVector.begin(); i != m_LODControllerVector.end();
       ++i) {
...
void CGraphicThingInstance::UpdateLODLevel() {
  ...
  if (!pcurCamera) {
    TraceError(
        "CGraphicThingInstance::UpdateLODLevel - GetCurrentCamera() == NULL");
    return;
  }
  ...
  update.fDistanceFromCenter =
      sqrtf((c_rv3TargetPosition.x - c_v3Position.x) *
                (c_rv3TargetPosition.x - c_v3Position.x) +
            (c_rv3TargetPosition.y - c_v3Position.y) *
                (c_rv3TargetPosition.y - c_v3Position.y));
  update.fDistanceFromCamera =
      sqrtf((c_rv3CameraPosition.x - c_v3Position.x) *
                (c_rv3CameraPosition.x - c_v3Position.x) +
            (c_rv3CameraPosition.y - c_v3Position.y) *
                (c_rv3CameraPosition.y - c_v3Position.y) +
            (c_rv3CameraPosition.z - c_v3Position.z) *
                (c_rv3CameraPosition.z - c_v3Position.z));
...
void CGraphicThingInstance::UpdateTime() {
  ...
  // m_fSecondElapsed = GrannyGetSecondsElapsed(&m_clockLast, &clockNow) *
  // m_fMotionTimeSpeed;
  ...
  m_fAverageSecondElapsed = m_fAverageSecondElapsed +
                            (m_fSecondElapsed - m_fAverageSecondElapsed) / 4.0f;
...
void CGraphicThingInstance::OnRender() { RenderWithOneTexture(); }

void CGraphicThingInstance::OnBlendRender() { BlendRenderWithOneTexture(); }
...
CGraphicThingInstance::GetLODControllerPointer(DWORD dwModelIndex) const {
  assert(dwModelIndex < m_LODControllerVector.size());
  return m_LODControllerVector[dwModelIndex];
}

CGrannyLODController *
CGraphicThingInstance::GetLODControllerPointer(DWORD dwModelIndex) {
  assert(dwModelIndex < m_LODControllerVector.size());
  return m_LODControllerVector[dwModelIndex];
}
...
float CGraphicThingInstance::GetHeight() {
  ...
  CGrannyModelInstance *pModelInstance =
      m_LODControllerVector[0]->GetModelInstance();
  if (!pModelInstance)
    return 0.0f;
...
CGraphicThingInstance::CGraphicThingInstance() { Initialize(); }
