...
SecureCipher::~SecureCipher() { CleanUp(); }
...
bool SecureCipher::ComputeClientKeys(const uint8_t *server_pk) {
  ...
  // Client: tx_key is for sending TO server, rx_key is for receiving FROM
  // server
  if (crypto_kx_client_session_keys(m_rx_key, m_tx_key, m_pk, m_sk,
                                    server_pk) != 0) {
    return false;
  }
...
bool SecureCipher::ComputeServerKeys(const uint8_t *client_pk) {
  ...
  // Server: tx_key is for sending TO client, rx_key is for receiving FROM
  // client
  if (crypto_kx_server_session_keys(m_rx_key, m_tx_key, m_pk, m_sk,
                                    client_pk) != 0) {
    return false;
  }
...
void SecureCipher::ComputeChallengeResponse(const uint8_t *challenge,
...
bool SecureCipher::VerifyChallengeResponse(const uint8_t *challenge,
                                           const uint8_t *response) {
  // Verify the HMAC - peer should have used their tx_key (our rx_key) to
  // compute it
...
void SecureCipher::ApplyStreamCipher(void *buffer, size_t len,
  ...
  if (offset != 0 && len > 0) {
    ...
    crypto_stream_xchacha20_xor_ic(ks, ks, 64, stream_nonce, byte_counter / 64,
                                   key);
  ...
  if (len > 0) {
    crypto_stream_xchacha20_xor_ic(p, p, (unsigned long long)len, stream_nonce,
                                   byte_counter / 64, key);
    byte_counter += len;
  }
}
...
bool SecureCipher::EncryptToken(const uint8_t *plaintext, size_t len,
  ...
  if (crypto_aead_xchacha20poly1305_ietf_encrypt(
          ciphertext, &ciphertext_len, plaintext, len, nullptr, 0, nullptr,
          nonce_out, m_tx_key) != 0) {
    return false;
  }
...
bool SecureCipher::DecryptToken(const uint8_t *ciphertext, size_t len,
  ...
  if (crypto_aead_xchacha20poly1305_ietf_decrypt(
          plaintext, &plaintext_len, nullptr, ciphertext, len, nullptr, 0,
          nonce, m_rx_key) != 0) {
    return false;
  }
