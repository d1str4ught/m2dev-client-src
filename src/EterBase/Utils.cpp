...
const char *CreateTempFileName(const char *c_pszPrefix) {
  ...
  if (!GetTempFileNameW(wTempPath, wPrefix,
...
void GetFilePathNameExtension(const char *c_szFile, int len,
...
void GetFileNameParts(const char *c_szFile, int len, char *pszPath,
...
public:
  ...
  bool OnFolder(const char *c_szFilter, const char *c_szPathName,
...
void StringExceptCharacter(std::string *pstrString, const char *c_szCharacter) {
  ...
  while ((iNextPos = pstrString->find_first_of(c_szCharacter, icurPos)) >= 0) {
    ...
    std::string strBack =
        pstrString->substr(iNextPos + 1, pstrString->length() - iNextPos - 1);
